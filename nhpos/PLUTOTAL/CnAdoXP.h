...
public:
	...
	virtual	HRESULT	AddNew(CnVariant FieldList,CnVariant Values){
		...
		try {
			...
			errorMessage.Format(_T("ERROR AddNew %s  %s"), e.ErrorMessage(), description);
			m_hr = e.Error();
		}
		if (FAILED(m_hr)) {
			...
			{
				char xBuff[256];
				sprintf (xBuff, "%S", (LPCTSTR) errorMessage);
	...
	virtual HRESULT Close(void){ 
		...
		try {
			...
			TCHAR *description;
			description = bstrSource;
			errorMessage.Format(_T("ERROR Close %s  %s"), e.ErrorMessage(), description);
			m_hr = e.Error();
		}
		if (m_pConnection) {
	...
	virtual	HRESULT	Delete(long AffectRecords = 1){ 
		...
		try {
			...
			TCHAR *description;
			description = bstrSource;
			errorMessage.Format(_T("ERROR Delete %s  %s"), e.ErrorMessage(), description);
			m_hr = e.Error();
		}
		if (FAILED(m_hr)) {
			TRACE3("%S(%d): %s\n", __FILE__, __LINE__, errorMessage);
	...
	virtual	HRESULT	GetRows(long Rows,VARIANT Start,VARIANT Fields,VARIANT __RPC_FAR *pvar) {
		...
		catch(_com_error &e)
		{
			...
			TCHAR *description;
			description = bstrSource;
			errorMessage.Format(_T("ERROR GetRows %s  %s"), e.ErrorMessage(), description);
			m_hr = e.Error();
		}
		if (FAILED(m_hr)) {
			TRACE3("%S(%d): %s\n", __FILE__, __LINE__, errorMessage);
	...
	virtual	 HRESULT	GetRow(CnVariant Fields,CnVariant __RPC_FAR *pvar) {
		...
		try {
			...
			TCHAR *description;
			description = bstrSource;
			errorMessage.Format(_T("ERROR GetRow %s  %s"), e.ErrorMessage(), description);
			m_hr = e.Error();
		}
		if (FAILED(m_hr)) {
			TRACE3("%S(%d): %s\n", __FILE__, __LINE__, errorMessage);
	...
	virtual	HRESULT Open(CnVariant Source,_bstr_t ActiveConnection)
	{
		...
		try {
			...
			TCHAR *description;
			description = bstrSource;
			errorMessage.Format(_T("ERROR Open %s  %s"), e.ErrorMessage(), description);
			m_hr = e.Error();
		}           
		if (!FAILED(m_hr)) {
			m_bOpened = TRUE;
			...
			TRACE3("%S(%d): %s", __FILE__, __LINE__, errorMessage);
			PifLog(MODULE_PLU_TOTAL, LOG_ERROR_TOTALUM_OPEN);
			PifLog(MODULE_ERROR_NO(MODULE_PLU_TOTAL), (USHORT)abs(m_hr));
			{
				char  xBuffer[512];

				sprintf(xBuffer, "LOG_ERROR_TOTALUM_OPEN: %S", (LPCTSTR) errorMessage);
	...
	virtual	 HRESULT STDMETHODCALLTYPE Update(CnVariant Fields, CnVariant Values) {
		...
		try {
			...
			TCHAR *description;
			description = bstrSource;
			errorMessage.Format(_T("ERROR Update %s  %s"), e.ErrorMessage(), description);
			m_hr = e.Error();
		}
		if (FAILED(m_hr)) {
			TRACE3("%S(%d): %s\n", __FILE__, __LINE__, errorMessage);
	...
	virtual	HRESULT	get_RecordCount(long __RPC_FAR *pVal){
		...
		try {
			...
			TCHAR *description;
			description = bstrSource;
			errorMessage.Format(_T("ERROR get_RecordCount %s  %s"), e.ErrorMessage(), description);
			m_hr = e.Error();
		}
		if (FAILED(m_hr)) {
			TRACE3("%S(%d): %s\n", __FILE__, __LINE__, errorMessage);
