...
CDevice::CDevice()
{
	...
	m_Caps[0].dwDeviceType = SCF_TYPE_MSR;
	...
	m_Serial[0].dwByteSize  = SCF_CAPS_CHAR_8;
	m_Serial[0].dwParity    = SCF_CAPS_PARITY_NONE;
...
UINT CDevice::DeviceThread()
{
	...
	while (bDoMyJob)						// do my job forever
	{
		...
		{
			case EVENT_MSR_DATA:				// scale event ?
				nStatus = OnDeviceEvent();
				break;
...
BOOL CDevice::OnInitialize()
{
	...
	m_hDevice   = ::CreateEvent(NULL, TRUE, FALSE, NULL);
...
HANDLE CDevice::OnOpen(LPCTSTR lpszDeviceName)
{
	...
	bDevice   = (BOOL)(m_Caps[nIndex].dwDeviceType & SCF_TYPE_MSR);
	...
	bResult = UieMSRInit(usPort, usBaud, uchFormat, m_hDevice);
...
DWORD CDevice::OnClose(HANDLE hDevice)
{
	...
	if (m_pDevice)
	{
		m_pDevice->m_nStatus = DEVICEIO_E_FAILURE;
		m_pDevice->Dispose();				// dispose it
...
DWORD CDevice::OnWrite(HANDLE hDevice, LPVOID lpBuffer, DWORD dwNumberOfBytesToWrite, LPDWORD lpNumberOfBytesWritten)
{
	Sleep (50);
...
DWORD CDevice::OnRead(HANDLE hDevice, LPVOID lpBuffer, DWORD dwNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead)
{
	PDEVICEIO_MSR_DATA	    pDevice;
	UIE_MSR 				Driver;
	SHORT					sResult;
	...
	// check parameters
	if (lpBuffer == NULL || dwNumberOfBytesToRead < sizeof(DEVICEIO_MSR_DATA) || lpNumberOfBytesRead == NULL)
	{
		Sleep (50);
		return DEVICEIO_E_ILLEGAL;			// exit ...
	}
	...
	sResult = UieReadMSR(&Driver);

	*lpNumberOfBytesRead = 0;

	if (sResult == DEVICEIO_SUCCESS) {
		// make up response data
		pDevice = (PDEVICEIO_MSR_DATA)lpBuffer;
		memset (pDevice, 0, sizeof(DEVICEIO_MSR_DATA));
		pDevice->sLength1 = Driver.uchLength1;
		if (pDevice->sLength1 > 0) {
			memcpy (pDevice->uchTrack1, Driver.auchTrack1, pDevice->sLength1 * sizeof(Driver.auchTrack1[0]));
			pDevice->sStatus1 = DEVICEIO_MSR_SUCCESS;
		}
		else {
			pDevice->sStatus1 = DEVICEIO_MSR_NO_DATA;
		}

		pDevice->sLength2 = Driver.uchLength2;
		if (pDevice->sLength2 > 0) {
			memcpy (pDevice->uchTrack2, Driver.auchTrack2, pDevice->sLength2 * sizeof(Driver.auchTrack2[0]));
			pDevice->sStatus2 = DEVICEIO_MSR_SUCCESS;
		}
		else {
			pDevice->sStatus2 = DEVICEIO_MSR_NO_DATA;
		}

		pDevice->sLength3 = Driver.uchLengthJ;
		if (pDevice->sLength3 > 0) {
			memcpy (pDevice->uchTrack3, Driver.auchTrackJ, pDevice->sLength3 * sizeof(Driver.auchTrackJ[0]));
			pDevice->sStatus3 = DEVICEIO_MSR_SUCCESS;
		}
		else {
			pDevice->sStatus3 = DEVICEIO_MSR_NO_DATA;
		}

		*lpNumberOfBytesRead = sizeof(DEVICEIO_MSR_DATA);
	}
	return sResult;
}
	...
	LPDWORD lpBytesReturned)
{
	...
	{
		case DEVICEIO_CTL_MSR_READTRACKS:
			dwResult = OnIoControlReadTracks(hDevice,dwIoControlCode,lpInBuffer,dwInBufferSize,lpOutBuffer,dwOutBufferSize,lpBytesReturned);
			break;
		case DEVICEIO_CTL_MSR_ENABLE:
		default:
...
// Function:	
//
// Prototype:	DWORD		CDevice::OnIoControlReadTracks(dwIoControlCode,
//								lpInBuffer, dwInBufferSize,
//								lpOutBuffer, dwOutBufferSize,
//								lpBytesReturned);
//
...
DWORD CDevice::OnIoControlReadTracks(HANDLE hDevice, DWORD dwIoControlCode,
	LPVOID lpInBuffer, DWORD dwInBufferSize,
	LPVOID lpOutBuffer, DWORD dwOutBufferSize,
	LPDWORD lpBytesReturned)
{
	...
	// check parameters
	if (lpOutBuffer == NULL || dwOutBufferSize < sizeof(DEVICEIO_MSR_DATA) || lpBytesReturned == NULL)
	{
		return DEVICEIO_E_ILLEGAL;			// exit ...
	}
	...
	*lpBytesReturned = 0;
	...
	if (sResult == DEVICEIO_SUCCESS) {
		// make up response data
		pDevice = (PDEVICEIO_MSR_DATA)lpOutBuffer;
		memset (pDevice, 0, sizeof(DEVICEIO_MSR_DATA));
		pDevice->sLength1 = Driver.uchLength1;
		if (pDevice->sLength1 > 0) {
			memcpy (pDevice->uchTrack1, Driver.auchTrack1, pDevice->sLength1 * sizeof(Driver.auchTrack1[0]));
		...
		*lpBytesReturned = sizeof(DEVICEIO_MSR_DATA);
	}
	...
	LPDWORD lpNumberOfBytesRead)
{
	...
	// check parameters
	if (lpszDeviceName == NULL || lpszDataName == NULL || lpData == NULL || dwNumberOfBytesToRead == 0 || lpNumberOfBytesRead == NULL)
	{
		return DEVICEIO_E_ILLEGAL;			// exit ...
	}
	...
	dwResult = ScfGetParameter(lpszDeviceName, lpszDataName, lpDataType, (LPVOID)achBuffer, sizeof(achBuffer), lpNumberOfBytesRead);
	...
	LPDWORD lpNumberOfBytesWritten)
{
	...
	// check parameters
	if (lpszDeviceName == NULL || lpszDataName == NULL || lpData == NULL || dwNumberOfBytesToWrite == 0 || lpNumberOfBytesWritten == NULL)
	{
		return DEVICEIO_E_ILLEGAL;			// exit ...
	}
...
//
// Prototype:	DWORD	CDevice::OnDeviceEvent();
//
// Inputs:		nothing
//
// Outputs:		DWORD	
...
DWORD CDevice::OnWaitForEvent(BOOL bPacket)
{
	...
	else if (dwResult == (WAIT_OBJECT_0 + 0))
	{
		...
		dwEvent = EVENT_MSR_DATA;
		::ResetEvent(m_hDevice);
	}
#if	defined(_WIN32_WCE)
...
BOOL CDevice::ReadParameter(int nIndex)
{
	...
	{
		dwResult = OnGetParameter(m_sName[nIndex], pSc->pchName, &dwDataType, &auchData, sizeof(auchData), &dwBytesRead);
...
USHORT CDevice::GetBaud(int nIndex)
{
	...
	{
		case SCF_CAPS_BAUD_300:		usBaud = 300;	break;
		case SCF_CAPS_BAUD_600:		usBaud = 600;	break;
		case SCF_CAPS_BAUD_1200:	usBaud = 1200;	break;
		case SCF_CAPS_BAUD_2400:	usBaud = 2400;	break;
		case SCF_CAPS_BAUD_4800:	usBaud = 4800;	break;
		case SCF_CAPS_BAUD_9600:	usBaud = 9600;	break;
		case SCF_CAPS_BAUD_19200:	usBaud = 19200;	break;
		default:					usBaud = 9600;	break;
	}
