#if !defined(AFX_FRAMEWORKWNDBUTTON_H__2B7DDBDD_24D5_44E4_B0FF_0C7B477F9C7B__INCLUDED_)
#define AFX_FRAMEWORKWNDBUTTON_H__2B7DDBDD_24D5_44E4_B0FF_0C7B477F9C7B__INCLUDED_
...
class CFrameworkWndButton : public CWindowButton
{
// Construction
public:
	CFrameworkWndButton();
	virtual ~CFrameworkWndButton();
...
public:
	...
	CFrameworkWndButton(int id, int row, int column, int width = 3, int height = 3);
	CFrameworkWndButton(CWindowControl *wc);
//	CFrameworkWndButton & operator = (const CDButtonEdit & cdt);
	...
	void DrawButtonOnDisplayFocus (CDC* pDC);
	void DrawButtonOnDisplayNoFocus (CDC* pDC);
	virtual int  ChangeRefreshIcon(TCHAR *tcsIconFileName);

	static void DrawButton(int nIDCtl, LPDRAWITEMSTRUCT lpDraw, CFrameworkWndItem* pWndItem);
/*
...
*/
	...
	static void sendButtonMessageToBL(CHARDATA* pCharData);//Sends data into BusinessLogic From Button Press

	static CFrameworkWndButton * TranslateCWindowControl (CWindowControl *pwc) {
		if (pwc->controlAttributes.m_nType == ButtonControl)
			return (static_cast <CFrameworkWndButton *> (pwc));
		else
			return NULL;
	}
...
#endif // !defined(AFX_FRAMEWORKWNDBUTTON_H__2B7DDBDD_24D5_44E4_B0FF_0C7B477F9C7B__INCLUDED_)
