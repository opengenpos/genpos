...
SHORT _RflRssVar(LABELANALYSIS *pData)
{
	...
	for(i = 3; i < _tcslen(pData->aszScanPlu);){
		switch(pData->aszScanPlu[i]){
		case _T('0'):	//GTIN n2 + n14
			if(pData->aszScanPlu[i + 1] == _T('1')){
				_tcsncpy(pData->aszMaskLabel, &pData->aszScanPlu[i + 2], LEN_RSS14);
				i += 16;
			}
			break;
		case _T('1'):
			switch(pData->aszScanPlu[i + 1]){
			case _T('1'):	//n2 + n6 Date
				//Production Date
				pData->uchDateType = LA_DATE_PROD;
				break;
			case _T('2'):	//n2 + n6 Date
				//Due Date
				pData->uchDateType = LA_DATE_DUE;
				break;
			case _T('3'):	//n2 + n6 Date
				//Packaging Date
				pData->uchDateType = LA_DATE_PACK;
				break;
			case _T('5'):	//n2 + n6 Date
				//Best Before date
				pData->uchDateType = LA_DATE_BEST;
				break;
			case _T('7'):	//n2 + n6 Date
				//expiration date
				pData->uchDateType = LA_DATE_EXP;
				break;
			}
			i += 2; //move past the application Identifier
			_tcsncpy(pData->aszDate, &pData->aszScanPlu[i], 6);
			i += 6;
			break;
		case _T('3'):
			switch(pData->aszScanPlu[i + 1]){
			case _T('1'):	//Metric Trade Measures
				switch(pData->aszScanPlu[i + 2]){
				case _T('0'):
					pData->ulMeasureType[0] = LA_QTY_WEIGHT | LA_QTY_KG;
					i += 3;
					pData->ulMeasureType[0] |= pData->ulMeasureType[0] | (LA_DEC_0 << (pData->aszScanPlu[i] - _T('0')));
					i++;
					_tcsncpy(pData->aszMeasurement[0], &pData->aszScanPlu[i], 6);
...
*/
				...
				break;
			case _T('2'): //Non-Metric Trade Measures
				switch(pData->aszScanPlu[i + 2]){
				case _T('0'):
...
*/				}
			...
			break;
/* //RPH Only use Weight
			case _T('5'): //Non-Metric Trade Measures
				switch(pData->aszScanPlu[i + 2]){
				case _T('0'):
				...
				break;
			case _T('6'): //Non-Metric Trade Measures
				switch(pData->aszScanPlu[i + 2]){
				case _T('0'):
...
*/
			i += 6;
			break;
			case _T('9'): //Amount/Price
				switch(pData->aszScanPlu[i + 2]){
				case _T('0'):
					i += 3;
					pData->uchPriceDecimal = pData->aszScanPlu[i] - _T('0');
					i++;
					break;
				case _T('1'):
					i += 3;
					pData->uchPriceDecimal = pData->aszScanPlu[i] - _T('0');
					i++;
					_tcsncpy(pData->aszISOCurCode, &pData->aszScanPlu[i], 3);
					i += 3;
					break;
				case _T('2'):
					i += 3;
