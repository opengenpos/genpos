...
*       PrtRecvOnAcnt_RJ() : prints received on acount (receipt & journal)
*       PrtRecvOnAcnt_VL() : prints received on acount (validation)
...
* Oct-28-93 : 02.00.02 : K.You      : del. PrtDflRecvOnAcnt()                                   
* Apr-08-94 : 00.00.04 : K.You      : add validation slip print feature.(mod. PrtRecvOnAcnt())
* Jan-17-94 : 03.00.00 : M.Ozawa    : PrtDflRecvOnAcnt() for display on the fly
...
VOID PrtRecvOnAcnt(TRANINFORMATION  *pTran, ITEMMISC  *pItem)
{
    ...
    if (   (fsPrtPrintPort & PRT_RECEIPT)     /* receipt, journal print */
        ...
        PrtRecvOnAcnt_RJ(pItem);
    }
}

/*
*===========================================================================
** Format  : VOID PrtRecvOnAcnt_RJ(ITEMMISC *pItem);      
*
...
** Return  : none
*            
** Synopsis: This function prints received on acount .(receipt & journal)
*===========================================================================
*/
VOID  PrtRecvOnAcnt_RJ(ITEMMISC *pItem)
{
    ...
    PrtRJAmtMnem(TRN_RA_ADR, pItem->lAmount); /* ROA line */
    
}

/*
*===========================================================================
** Format  : VOID PrtRecvOnAcnt_VL(TRANINFORMATION *pTran, ITEMMISC *pItem);      
*
...
** Return  : none
*            
** Synopsis: This function prints received on acount .(validation)
*===========================================================================
*/
VOID  PrtRecvOnAcnt_VL(TRANINFORMATION *pTran, ITEMMISC *pItem)
{
    ...
    if ( usPrtVLColumn == PRT_VL_24 ) {
        ...
        PrtVLAmtMnem(TRN_RA_ADR, pItem->lAmount);   /* send amount line */
    ...
    if ( usPrtVLColumn == PRT_VL_21 ) {
        ...
        PrtVSAmtMnem(pTran, pItem->fbModifier, TRN_RA_ADR, pItem->lAmount);
...
/*
*===========================================================================
** Format  : VOID PrtMoney_RJ(ITEMMISC *pItem);
*
*   Input  : ITEMMISC         *pItem     -Item Data address
*   Output : none
*   InOut  : none
** Return  : none
*
** Synopsis: This function prints money.(receipt & journal)
*===========================================================================
*/
VOID  PrtMoney_RJ(ITEMMISC *pItem)
{
    USHORT usFor = 0;  /* set 0 , not to print FOR Value */
    UCHAR  uchSymAdr;
    USHORT usTrnsAdr;
    ...
    if((pItem->uchTendMinor >= CLASS_FOREIGN1) && (pItem->uchTendMinor <= CLASS_FOREIGN8)) {
        PrtRJForeignQtyFor(pItem->lQTY, pItem->lUnitPrice, uchSymAdr, pItem->fchStatus);
        PrtRJMoneyForeign(pItem->lAmount, usTrnsAdr, uchSymAdr, pItem->fchStatus);
    } else if ((pItem->uchTendMinor >= CLASS_TENDER1) && (pItem->uchTendMinor <= CLASS_TENDER8)) {
        PrtRJQtyFor(pItem->lQTY, usFor, pItem->lUnitPrice);
        PrtRJAmtMnem(((pItem->uchTendMinor - CLASS_TENDER1) + TRN_TEND1_ADR), pItem->lAmount);
    } else if((pItem->uchTendMinor >= CLASS_TENDER9) && (pItem->uchTendMinor <= CLASS_TENDER11)) {
        PrtRJQtyFor(pItem->lQTY, usFor, pItem->lUnitPrice);
        PrtRJAmtMnem(((pItem->uchTendMinor - (UCHAR)CLASS_TENDER9) + TRN_TEND9_ADR), pItem->lAmount);
    } else if((pItem->uchTendMinor >= CLASS_TENDER12) && (pItem->uchTendMinor <= CLASS_TENDER20)) {
        PrtRJQtyFor(pItem->lQTY, usFor, pItem->lUnitPrice);
        PrtRJAmtMnem(((pItem->uchTendMinor - CLASS_TENDER12) + TRN_TENDER12_ADR), pItem->lAmount);
    }
}
...
VOID    PrtMoney_SP(TRANINFORMATION *pTran, ITEMMISC *pItem)
{
    ...
    USHORT  usSlipLine = 0,            /* number of lines to be printed */
            ...
            i,
            usFor = 0;                 /* set 0 , not to print FOR Value */
    ...
    /* -- set mnemonic -- */
    if(pItem->uchTendMinor >= CLASS_FOREIGN1 && pItem->uchTendMinor <= CLASS_FOREIGN8) {
        usSlipLine += PrtSPForeignQtyFor(aszSPPrintBuff[usSlipLine], pItem->lQTY, pItem->lUnitPrice, uchSymAdr, pItem->fchStatus);

        usSlipLine += PrtSPMoneyForeign(aszSPPrintBuff[usSlipLine], pItem->lAmount, usTrnsAdr, uchSymAdr, pItem->fchStatus);
    } else {
        /* -- set qty */
        usSlipLine += PrtSPQtyFor(aszSPPrintBuff[usSlipLine], pItem->lQTY, usFor, pItem->lUnitPrice);
		{
			USHORT usTenderTranAddress = 1;

			if (pItem->uchTendMinor <= CLASS_TENDER8) {
				usTenderTranAddress = pItem->uchTendMinor - CLASS_TENDER1 + TRN_TEND1_ADR;
			} else if (pItem->uchTendMinor >= CLASS_TENDER9 && pItem->uchTendMinor <= CLASS_TENDER11) {
				usTenderTranAddress = pItem->uchTendMinor - CLASS_TENDER9 + TRN_TEND9_ADR;
			} else  if (pItem->uchTendMinor >= CLASS_TENDER12 && pItem->uchTendMinor <= CLASS_TENDER20) {
				usTenderTranAddress = pItem->uchTendMinor - CLASS_TENDER12 + TRN_TENDER12_ADR;
			}
			usSlipLine += PrtSPMnemAmt(aszSPPrintBuff[usSlipLine], usTenderTranAddress, pItem->lAmount);
		}
    }
    /* -- check if paper change is necessary or not -- */
    usSaveLine = PrtCheckLine(usSlipLine, pTran);
...
/*
*===========================================================================
** Format  : VOID  PrtDflMoney(TRANINFORMATION  *pTran, ITEMMISC *pItem)
*
*   Input  : TRANINFORMATION  *pTran     -Transaction Information address
*            ITEMMISC         *pItem     -Item Data address
*   Output : none
...
*===========================================================================
*/
VOID    PrtDflMoney(TRANINFORMATION *pTran, ITEMMISC *pItem)
{
    TCHAR   aszDflBuff[8][PRT_DFL_LINE + 1]; /* display data save area */
    USHORT  usLineNo;                       /* number of lines to be displayed */
    USHORT  usOffset = 0;                       
    ...
    usLineNo += PrtDflAmtMnem(aszDflBuff[usLineNo], usTrnsAdr, pItem->lAmount); 
