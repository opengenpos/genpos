...
* Mar-06-95 : 03.00.00 : T.Nakahata : Add Unique Trans No. (mod PrtDflHeader)
...
* Jun-03-96 : 03.01.08 : M.Ozawa    : Del Unique Trans No. for ISV KDS trouble
* 2171 for Win32
...
VOID    PrtDflInit(TRANINFORMATION  *pTran)
{
    ...
    //memset(PrtDflIf.Dfl.DflHead.aszTermAdr, '0', PRT_DFL_TMLEN - usLen);
    ...
    //memset(PrtDflIf.Dfl.DflHead.aszConsNo, '0', PRT_DFL_CONSLEN - usLen);
    //memcpy(&PrtDflIf.Dfl.DflHead.aszConsNo[PRT_DFL_CONSLEN - usLen], aszConsNo, usLen);
...
//        usLineNo += PrtDflTblPerson( aszDflBuff[ usLineNo ], pTran->TranGCFQual.usTableNo, 0, PRT_DOUBLE );

        / -- set destination CRT -- /

        PrtDflIf.Dfl.DflHead.auchCRTNo[0] = 0x30;   / CRT#1 - #4 /
        PrtDflIf.Dfl.DflHead.auchCRTNo[1] = 0x30;   / CRT#5 - #8 /
...
*
*   Input  : UCHAR  *puchCRTNo     -destination CRT No.
*   Output : none
*   InOut  : none
...
VOID   PrtDflIType(USHORT usLineNo, UCHAR uchItemType)
{
    ...
    //memset(PrtDflIf.Dfl.DflHead.aszLineNo, '0', PRT_DFL_LINELEN - usLen);
    //memcpy(&PrtDflIf.Dfl.DflHead.aszLineNo[PRT_DFL_LINELEN - usLen], aszLineNo, usLen);
...
VOID   PrtDflSend(VOID)
{
    ...
    usDataLen = tcharlen(PrtDflIf.auchDispData);
    ...
    //memset(PrtDflIf.Dfl.DflHead.aszVLI, '0', PRT_DFL_VLILEN - usLen);
    //memcpy(&PrtDflIf.Dfl.DflHead.aszVLI[PRT_DFL_VLILEN - usLen], aszVLI, usLen);
