...
VOID  PrtThrmSupCPN( PARACPN *pData )
{
    ...
    TCHAR  aszAddress1[5] = { 0 };
    TCHAR  aszAddress2[5] = { 0 };
    TCHAR  aszAddress3[5] = { 0 };
    TCHAR  aszAddress4[5] = { 0 };

	// change to use the standard number of coupons #define STD_MAX_NUM_PLUDEPT_CPN
    TCHAR  aszPLUNoAll[STD_MAX_NUM_PLUDEPT_CPN][PLU_MAX_DIGIT + 1] = { 0 };

	for (USHORT iLoop = 0; iLoop < STD_MAX_NUM_PLUDEPT_CPN; iLoop++) {
    ...
    if(pData->usPrintControl & PRT_RECEIPT){
        ...
        USHORT  usStdMaxNumPluDeptCpn_ByTwos = (STD_MAX_NUM_PLUDEPT_CPN & 0xffe);  // remove the least significant bit if it should be 1 to print by twos.
        ...
        /*
         * print president PLU Number and Adjective Code in rows of two.
         * we loop through the first even numbered and then print the last,odd one
         * should it exist.
         */
		for (USHORT iLoop = 0; iLoop < usStdMaxNumPluDeptCpn_ByTwos; iLoop += 2) {
			PrtPrintf(PMG_PRT_RECEIPT,auchPrtThrmSupCPN3,
					  aszPLUNoAll[iLoop], ( USHORT)(pData->uchCPNAdj[iLoop] & 0x0F), ( USHORT)((pData->uchCPNAdj[iLoop] & 0xF0) >>4),
					  aszPLUNoAll[iLoop + 1], ( USHORT)(pData->uchCPNAdj[iLoop + 1] & 0x0F), ( USHORT)((pData->uchCPNAdj[iLoop + 1] & 0xF0) >>4));
		}
