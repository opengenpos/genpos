...
PMGBLK  PmgConnEngineBlk[MAX_PRINTER];
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineWinInitialize(VOID)
*
**Return:       Non
*
...
VOID PmgConnEngineInitialize( VOID )
{
    ...
    {
        PmgConnEngineBlk[i].dwDllLoaded = FALSE;
    }
    ...
    for (i = 0; i < dwCount; i++)
    {
        PmgConnEngineBlk[i].dwDllLoaded = TRUE;
    }
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineFinalize(VOID)
*
**Return:       Non
*
...
VOID PmgConnEngineFinalize( VOID )
{
    ...
    for (i = 0; i < MAX_PRINTER; i++)
    {
        if (PmgConnEngineBlk[i].dwDllLoaded)
        {
            PmgConnEngineBlk[i].Func.Close(PmgConnEngineBlk[i].hHandle);
            FreeDevice();
        }
    }
#endif
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEnginePrint(usPrtType, pucBuff, usLen)
...
//	TCHAR  *pPrtDoubleS = _T("! U1 SETLP 7 3 60\r\n! U1 SETSP 5\r\n");
    ...
    if(pucBuff[0] == ESC) {
		// If we hit the "Paper Cut" escape sequence
		// then send the CUT sentinel value to the
		// device engine, this will send the entire
		// buffer to the printer
		if (_tcscmp(pucBuff,_T("\x1B|75P")) == 0 )
		{
			prtBuff[0] = _T('C');
			prtBuff[1] = _T('U');
			prtBuff[2] = _T('T');
			...
			*/
			...
			if ( ( usPrtTypeMasked == PMG_PRT_RECEIPT) ||
				 ...
				 ( usPrtTypeMasked == PMG_PRT_SLIP) ) {
					...
					PmgConnEngineGetStatus(usPrtType, &fbPrtStatus);
...
**************************************************************************
*
**Synopsis:     VOID    PmgConnEngineWait( VOID )
*
**Return:       non
*
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineBeginSmallValidation(usPrtType)
*               input   USHORT  usPrtType;  : type of printer
*
**Return:       PMG_ERROR_PROVIDE   : printer not provide
*
...
USHORT PmgConnEngineBeginSmallValidation(USHORT usPrtType)
{
#if 0
    DWORD   i;
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineEndSmallVlidation(usPrtType)
*               input   USHORT  usPrtType;  : type of printer
*
**Return:       PMG_ERROR_PROVIDE   : printer not provide
*
...
USHORT PmgConnEngineEndSmallValidation(USHORT usPrtType)
{
#if 0
    DWORD   i;
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineBeginValidation(usPrtType)
*               input   USHORT  usPrtType;  : type of printer
*
**Return:       PMG_ERROR_PROVIDE   : printer not provide
*
...
USHORT PmgConnEngineBeginValidation(USHORT usPrtType)
{
#if 0
    DWORD   i;
    DEVICEIO_PRINTER_ARG_VALIDATION Arg;
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineEndVlidation(usPrtType)
*               input   USHORT  usPrtType;  : type of printer
*
**Return:       PMG_ERROR_PROVIDE   : printer not provide
*
...
USHORT PmgConnEngineEndValidation(USHORT usPrtType)
{
#if 0
    DWORD   i;
    DEVICEIO_PRINTER_ARG_END_VALIDATION Arg;
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineFeed(usPrtType, usLine)
*               input   USHORT  usPrtType;  : type of printer
...
USHORT PmgConnEngineFeed(USHORT usPrtType, USHORT usLine)
{
    USHORT   i = 0, j;
	TCHAR	tchBuff[8] = {0};
    ...
    {
		PmgConnEnginePrint(usPrtType, tchBuff, i);
    }
    return(PMG_SUCCESS);
}
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineBeginImportant(usPrtType)
*               input   USHORT  usPrtType;  : type of printer
*
**Return:       PMG_ERROR_PROVIDE   : printer not provide
*
...
USHORT PmgConnEngineBeginImportant(USHORT usPrtType)
{
    return(PMG_SUCCESS);
}
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineEndImportant(usPrtType)
*               input   USHORT  usPrtType;  : type of printer
*
**Return:       PMG_ERROR_PROVIDE   : printer not provide
*
...
USHORT PmgConnEngineEndImportant(USHORT usPrtType)
{
    return(PMG_SUCCESS);
}
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineGetStatus(usPrtType, pfbStatus)
*               input   USHORT  usPrtType;  : type of printer
...
USHORT PmgConnEngineGetStatus(USHORT usPrtType, USHORT *pfbStatus)
{
    DWORD   i;
...
USHORT PmgConnEngineGetStatusOnly(USHORT usPrtType, USHORT *pfbStatus)
{
    DWORD   i;
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEnginePrtConfig(usPrtType, pusColumn, pfbStatus)
*               input   USHORT  usPrtType;  : type of printer
...
USHORT PmgConnEnginePrtConfig(USHORT usPrtType, USHORT *pusColumn, USHORT *pfbStatus)
{
    DWORD   i;
	USHORT  usPrtTypeMasked = PMG_GET_PRT_STATION(usPrtType);
...
**************************************************************************
*
**Synopsis:     USHORT  PmgConnEngineFont(usPrtType, usFont)
*               input   USHORT  usPrtType;  : type of printer
...
USHORT  PmgConnEngineFont( USHORT usPrtType, USHORT usFont)
{
    DWORD   i;
	USHORT  usPrtTypeMasked = PMG_GET_PRT_STATION(usPrtType);
...
//            PmgConnEngineBlk[i].Func.IoControl(
//                PmgConnEngineBlk[i].hHandle, 
...
**************************************************************************
*
**Synopsis:     VOID    PmgConnEngineSetValWaitCount(USHORT usRetryCount)
...
VOID PmgConnEngineSetValWaitCount(USHORT usRetryCount)
{
    DWORD   i;
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineVersion(VOID)
*
**Input:		None
*
**Return:       None
...
VOID PmgConnEngineVersion( VOID )
{
	DWORD i;
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineSerialConnection(LPCTSTR szPort, long lBaud)
*
...
VOID PmgConnEngineSerialConnection( LPCTSTR szPort, long lBaud )
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineTcpConnection(LPCTSTR szHostname, long lPort)
*
...
*********************************************************************
*/
VOID  PmgConnEngineTcpConnection(LPCTSTR szHostname, long iPort)
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineIrDAConnection()
*
**Input:		NONE
*
**Return:       None
...
*********************************************************************
*/
VOID  PmgConnEngineIrDAConnection()
{
...
*********************************************************************
*
**Synopsis:     LONG    PmgConnEnginePrint(LPCTSTR szData)
*
...
*********************************************************************
*/
ULONG  PmgConnEnginePrint(LPCTSTR szData)
{
...
*********************************************************************
*
**Synopsis:     LONG    PmgConnEngineOpen()
*
**Input:		NONE
*
**Return:       This function returns PMG_SUCCESS on success and 
*				PMG_ERROR_PRINTER if PmgConnEngineOpen() failed. 
*
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineClose()
*
**Input:		NONE
*
**Return:       NONE
...
*********************************************************************
*
**Synopsis:     LONG    PmgConnEngineGetLastError()
*
**Input:		NONE
*
...
*********************************************************************
*/
ULONG  PmgConnEngineGetLastError()
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineShowLastError()
*
**Input:		NONE
*
**Return:       NONE
...
*********************************************************************
*/
VOID  PmgConnEngineShowLastError()
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineShowPrompts(long bEnablePrompts)
*
...
*********************************************************************
*/
VOID  PmgConnEngineShowPrompts(long bEnablePrompts)
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineEnableStatusCheck(long bEnableCheck)
*
...
*********************************************************************
*/
VOID  PmgConnEngineEnableStatusCheck(long bEnableCheck)
{
...
*********************************************************************
*
**Synopsis:     LONG    PmgConnEngineSend(LPCTSTR data)
*
...
*********************************************************************
*/
ULONG  PmgConnEngineSend(LPCTSTR data)
{
...
*********************************************************************
*
**Synopsis:     LONG    PmgConnEngineGetPrinterStatus()
*
**Input:		NONE
*
...
*********************************************************************
*/
ULONG  PmgConnEngineGetPrinterStatus()
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineSetStatusQueryTimeout(long lTimeout)
*
...
*********************************************************************
*/
VOID  PmgConnEngineSetStatusQueryTimeout(long lTimeout)
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineEnablePrintVerify(long bVerify)
*
...
*********************************************************************
*/
VOID  PmgConnEngineEnablePrintVerify(long bVerify)
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineSerialCloseWait(long lWaitTime)
*
...
*********************************************************************
*/
VOID  PmgConnEngineSerialCloseWait(long lWaitTime)
{
...
*********************************************************************
*
**Synopsis:     CSTRING    PmgConnEngineRecv(long lTimeout)
*
...
*********************************************************************
*/
UCHAR  PmgConnEngineRecv(long lTimeout)
{
...
*********************************************************************
*
**Synopsis:     VOID    PmgConnEngineSetFlowCtl(long lFlowCtlMask)
*
...
*********************************************************************
*/
VOID  PmgConnEngineSetFlowCtl(long lFlowCtlMask)
{
