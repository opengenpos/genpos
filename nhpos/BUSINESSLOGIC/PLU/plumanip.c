...
* 10/29/18 : 02.02.02  : R.Chambers  : replace Rfl_SpBsearch_PLU32() with new ULONG version of Rfl_SpBsearch().
\*==========================================================================*/
...
#include	<pif.h>								/* 2170 system header		*/
...
#ifdef PEP
#define     RECIDX_BUFFER       500         /* buffer size for index search */
#else
...
\*==========================================================================*/
/*							measure:	12345678							*/
static	TCHAR FARCONST	szPluFile[]  = _T("PARAM$PL");	/* PLU file name		*/
static	TCHAR FARCONST	szMixFile[]  = _T("PARAM$MX");	/* mix-match file		*/
/*static	UCHAR FARCONST	szPpiFile[]  = "PARAM$PP";*//* PPI file name		*//* ### DEL 2172 Rel1.0 */
static	TCHAR FARCONST	szPluMaint[] = _T("PARAM!PL");	/* PLU maintenance		*/
static	TCHAR FARCONST	szPluIndex[] = _T("PARAMIDX");	/* PLU index file,R20	*/
...
static	USHORT	AddRecord(USHORT, ULONG, UCHAR *, PFILEHDR);	/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
static	USHORT	CheckAdjective(UCHAR *, UCHAR *, PKEYINF, SHORT *);
...
static	USHORT	DelRecHash(USHORT, UCHAR *, USHORT, PKEYINF, PFILEHDR);
static	USHORT	DelRecRelative(USHORT, UCHAR *, PKEYINF, PFILEHDR);
static	USHORT	DelRecIndex(USHORT, VOID *, PFILEHDR);
static	USHORT	ReadRecHash(USHORT, UCHAR *, TCHAR *, USHORT, PKEYINF, PFILEHDR, ULONG *);	/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
static	USHORT	ReadRecRelative(USHORT, UCHAR *, UCHAR *, PKEYINF, PFILEHDR, ULONG *);		/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
static  USHORT	ReadRecIndex(USHORT, UCHAR *, UCHAR *, PFILEHDR, ULONG *);					/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
static	USHORT	TraceRecHash(USHORT, UCHAR *, PKEYINF, PFILEHDR, PSEQMNP);
static	USHORT	TraceRecRelative(USHORT, UCHAR *, PKEYINF, PFILEHDR, PSEQMNP);
static	USHORT	TraceRecMainte(USHORT, UCHAR *, PKEYINF, PFILEHDR, PSEQMNP);
static  USHORT	TraceRecIndex(USHORT, USHORT, USHORT, UCHAR *, PKEYINF, PFILEHDR, PSEQMNP);

/* --- misc. sub-functions --- */

#if 0
#pragma message("  ====++++====   EvaluateFileInfo_Debug() is ENABLED    ====++++====\z")
static	USHORT	EvaluateFileInfo_Debug(USHORT, USHORT *, PINFTBL *, char *, int);
static	PINFTBL	SearchFileInfo_Debug(USHORT, char *, int);
#define EvaluateFileInfo(usF,pusH,pTbl) EvaluateFileInfo_Debug(usF,pusH,pTbl,__FILE__,__LINE__)
#define SearchFileInfo(usF) SearchFileInfo_Debug(usF,__FILE__,__LINE__)
#else
...
static	ULONG	ComputeHash(TCHAR FAR *, USHORT, ULONG);			/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
static	ULONG	ComputeRelative(ULONG);							/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */

static  SHORT	ComputeIndex(UCHAR *, USHORT, PFILEHDR, ULONG *, UCHAR *);

ULONG	ComputeHash_64(TCHAR *, USHORT, ULONG);	/* ### ADD 2172 Rel1.0 (PLU No 7 -> 13) */

/* --- data tables --- */

INFTBL FARCONST		FileInfo[];					/* file information array	*/
...
*==========================================================================
*/
static SHORT    CompIndex(RECIDX *pKeyRecIdx, RECIDX *pHitPoint)
{
...
;========================================================================
**/
#if defined(MnpGetFileInfo)
USHORT	MnpGetFileInfo_Special(USHORT usFile, PSPECS pSpec);

USHORT	MnpGetFileInfo_Debug(USHORT usFile, PSPECS pSpec, char *aszFilePath, int nLineNo)
{
	int iLen = 0;
	char  xBuffer[256];
	...
	sprintf (xBuffer, "  MnpGetFileInfo_Debug(): hf = %d, File %s, lineno = %d", usFile, aszFilePath + iLen, nLineNo);
	NHPOS_ASSERT_TEXT(0, xBuffer);
	return MnpGetFileInfo_Special(usFile, pSpec);
}
...
#endif
{
	...
	/* --- evaluate file information --- */
	if (SPLU_COMPLETED == (usStat = EvaluateFileInfo(usFile, &usHandle, &pTable))) {
		FILEHDR		hdrFile = {0};

		/* --- get information of the file --- */
		if (SPLU_COMPLETED == (usStat = PhyGetInfo(usHandle, &hdrFile))) {
			/* --- respond it --- */
...
USHORT	MnpDeleteRecord(USHORT usFile, UCHAR *pucMainKey, USHORT usSubKey)
{
	USHORT		usStat, usHandle;
	FILEHDR		hdrFile;
	PINFTBL		pTable;
					  ...
					  UCHAR  *pucBuffer)
{
	...
	if (SPLU_COMPLETED == usStat && pManip) {					/* require to reserve it ?	*/
...
USHORT	MnpBeginLookUp(USHORT usFile, USHORT *pusManip, USHORT *pusSeq, USHORT usFileVer)
{
	...
	return ((i == ARRAYS(aManipBlock, SEQMNP)) ? SPLU_TOO_MANY_USERS : SPLU_COMPLETED);
}
...
USHORT	MnpReadFile(USHORT usManip, UCHAR *pucBuffer, ULONG ulSize, ULONG *pulRead)
{
	USHORT		usStat,  usHandle;
	ULONG		ulBytes;
	FILEHDR		hdrFile;
	...
	*pulRead = 0;								/* assume no data read		*/
	ulBytes  = ulSize;							/* no. of bytes to read		*/
	...
	usStat = PhyReadFile(usHandle, pucBuffer, &ulBytes, pManip->ulOffset, &ulActualBytesRead);

	/* --- completed ? --- */

	if (SPLU_COMPLETED == usStat) {				/* completed ?				*/
		*pulRead          = ulActualBytesRead;		/* respond no. of bytes read	*/
		pManip->ulOffset += ulActualBytesRead;		/* next file position	*/
	}
...
USHORT	MnpWriteFile(USHORT usMh, UCHAR *pucData, USHORT usBytes, USHORT *pusWritten)
{
	USHORT		usStat, usHandle;
	ULONG		ulLength;
	FILEHDR		hdrFile;
...
USHORT	MnpSeekFile(USHORT usManip, LONG lMoves, USHORT usOrigin, ULONG *pulNewPtr)
{
	...
	usStat = (ulOffset <= hdrFile.ulFileSize) ? SPLU_COMPLETED : SPLU_FILE_OVERFLOW;

	/* --- update file position --- */

	pManip->ulOffset = (usStat == SPLU_COMPLETED) ? ulOffset : pManip->ulOffset;
...
USHORT	MnpGetLookUp(USHORT usManip, UCHAR *pucBuffer)
{
	USHORT		usStat, usHandle;
	FILEHDR		hdrFile;
	PINFTBL		pTable;
...
USHORT	MnpModifyRecord(USHORT usManip, VOID *pvRec)
{
	...
	NHPOS_ASSERT(sizeof(aucRecord) >= pKey->usBytesRec);
	NHPOS_ASSERT(sizeof(RECPLU) >= pKey->usBytesRec);
...
USHORT	MnpRemoveRecord(USHORT usManip)
{
	...
	pChain = (PMNTCNT)(aucRecord + pKey->usBytesRec);
...
USHORT	MnpMoveFilePtr(USHORT usManip, VOID *pItemNumber)
{
	...
	if (SPLU_COMPLETED == usStat) {				/* require to reserve it ?	*/
...
USHORT	MnpOpenMirror(TCHAR FAR *pszFileName, USHORT *pusMirrorFile)
{
	USHORT		usStat, usHandle;
	FILEHDR		hdrFile;
	PINFTBL		pTable;
	...
	*pusMirrorFile = 0;							/* initialize handle value	*/
	...
	if (PhyOpenFile(pszFileName, &usHandle, TRUE) != SPLU_COMPLETED) {
		return (SPLU_FILE_NOT_FOUND);
	}

	/* --- audit the file --- */

	if ((usStat = PhyAuditFile(usHandle)) == SPLU_COMPLETED) {	/* is valid file struture ?	*/

		/* --- get file information --- */

		if ((usStat = PhyGetInfo(usHandle, &hdrFile)) == SPLU_COMPLETED) {
			...
			if (pTable = SearchFileInfo(hdrFile.usFileNumber)) {
				...
				if (pTable->usFileType == hdrFile.usFileType) {
					usStat = SPLU_COMPLETED;	/* OK ! Valid file given !	*/
				}
			}
		}
	}
	...
	if (SPLU_COMPLETED == usStat) {				/* is valid file given ?	*/
...
USHORT	AddRecHash(USHORT hf, VOID *pvRec, PKEYINF pKey, PFILEHDR pHeader)
{
	...
	UCHAR	aucNative[ENOUGHREC], aucPrev[ENOUGHREC];
	SHORT	sComp;
	USHORT	usStat, usFile;
	...
	pChainNative = (PRECCNT)(aucNative + pKey->usBytesRec);
	pChainPrev   = (PRECCNT)(aucPrev   + pKey->usBytesRec);
	fBrokenChain = FALSE;
	...
	if (pChainNative->fcInfo & REC_LEGAL_POS) {	/* is original record ?		*/
		...
		ulPrev   = ComputeHash(aucNative, pKey->usKeyLen, pHeader->ulHashKey); /* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
	}
...
USHORT	AddRecord(USHORT hf, ULONG ulCell, UCHAR *aucRec, PFILEHDR pHeader)
{
	USHORT	usStat;
...
USHORT	CheckAdjective(UCHAR *puc1st, UCHAR *puc2nd, PKEYINF pKey, SHORT *psComp)
{
	...
	if (pKey->usSubKeyType == TYPUCH) {			/* UCHAR type ?				*/
		us1stAdj = (USHORT)(*(puc1st + pKey->usSubKeyOffset));
		us2ndAdj = (USHORT)(*(puc2nd + pKey->usSubKeyOffset));
...
USHORT	AddRecRelative(USHORT hf, VOID *pvRec, PKEYINF pKey, PFILEHDR pHeader)
{
	...
	if (usStat = PhyWriteRecord(hf, ulCell, (UCHAR *)pvRec, pHeader)) {
		return (usStat);
	}
...
USHORT	AddRecIndex(USHORT hf, VOID *pvRec, PFILEHDR pHeader)
{
    ...
    ULONG   usBytes;
	USHORT	usStat;
    ULONG    ulHitOffset, ulWriteOffset;
	ULONG	ulActualBytesRead;
#ifndef PEP
	...
	sStatus = ComputeIndex(pvRec, hf, pHeader, &ulHitOffset, (UCHAR *)&aBuffer);
	...
	if (sStatus == SPLU_COMPLETED) {				/* is already located ?		*/
		return (SPLU_REC_DUPLICATED);				/* duplicated error !	*/
	}
...
#else
    d13ProdWork = sizeof(RECIDX) * pHeader->ulCurRec;
    ulWriteOffset += (ULONG)d13ProdWork;
#endif
    ...
    while(ulWriteOffset > ulHitOffset) {
        ...
        usStat = PhyReadFile(hf, (UCHAR *)&aBuffer, &usBytes, ulWriteOffset, &ulActualBytesRead);
            
    	usStat = PhyWriteFile(hf,                   /* write to a file			*/
						  (UCHAR *)&aBuffer,        /* data buffer ptr.		*/
				   ...
				   PFILEHDR pHeader)
{
	...
	UCHAR	aucPrev[ENOUGHREC], aucRecord[ENOUGHREC];
	...
	pChainPrev = (PRECCNT)(aucPrev   + pKey->usBytesRec);
	pChainRec  = (PRECCNT)(aucRecord + pKey->usBytesRec);
	...
	while (FOREVER) {
		...
		if (! memcmp(aucRecord, pucMainKey, pKey->usKeyLen)) {
			...
			if (pKey->usSubKeyType == TYPUCH) {	/* is UCHAR type ?			*/
				usAdj = (USHORT)(*(aucRecord + pKey->usSubKeyOffset));
			} else {							/* else, assume USHORT type	*/
				usAdj = *(USHORT *)(aucRecord + pKey->usSubKeyOffset);
			}
...
USHORT	DelRecRelative(USHORT hf, UCHAR *pucMainKey, PKEYINF pKey, PFILEHDR pHeader)
{
	BOOL	fOccupy;
	USHORT	usStat, usFile;
	ULONG	ulCell, ulKey;	/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
...
USHORT	DelRecIndex(USHORT hf, VOID *pvRec, PFILEHDR pHeader)
{
	...
	ULONG	 usBytes;
	USHORT	usStat;
	ULONG	ulHitOffset, ulWriteOffset, ulTailOffset;
	ULONG	ulActualBytesRead;
#ifndef PEP
    D13DIGITS  d13ProdWork;
#endif
...
#else
    d13ProdWork = sizeof(RECIDX) * (pHeader->ulCurRec - 1L);	/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
    ulTailOffset += (LONG)d13ProdWork;
#endif
    ulWriteOffset = ulHitOffset;
    ...
    while(ulWriteOffset < ulTailOffset) {
        ...
        usStat = PhyReadFile(hf, (UCHAR *)&aBuffer, &usBytes, ulWriteOffset+sizeof(RECIDX), &ulActualBytesRead);
...
USHORT	ReadRecHash(USHORT   hf, UCHAR *pucBuffer, TCHAR *pucMainKey, USHORT usSubKey,
					PKEYINF pKey, PFILEHDR pHeader, ULONG *pulCellInfo)
{
	BOOL	fOccupy, fAdjItem;
	UCHAR	aucRecord[ENOUGHREC*2];
	USHORT	usAdj, usStat;
	ULONG	ulCell;		/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
	PRECCNT	pChain;
	...
	pChain = (PRECCNT)(aucRecord + pKey->usBytesRec);
...
USHORT	ReadRecRelative(USHORT hf, UCHAR *pucBuffer, UCHAR *pucMainKey,
					PKEYINF  pKey, PFILEHDR pHeader, ULONG *pulCellInfo)
{
	BOOL	fOccupy;
	USHORT	usStat;
...
USHORT	ReadRecIndex(USHORT hf, UCHAR *pucBuffer, UCHAR *pvRec,
					PFILEHDR pHeader, ULONG *pulCellInfo)
{
	...
	ULONG	ulBytes;
	USHORT	usStat;
	ULONG	ulHitOffset, ulOffsetSave;
#ifndef PEP
    D13DIGITS  d13ProdWork;
	ULONG	   ulActualBytesRead;
#endif
...
#else
    d13ProdWork = ulHitOffset;
    d13ProdWork /= sizeof(RECIDX);
    ulHitOffset = (ULONG)d13ProdWork;
#endif
	*pulCellInfo = ulHitOffset;
    ...
    if (pucBuffer) {
        ...
        ulBytes = sizeof(RECIDX);

        usStat = PhyReadFile(hf, pucBuffer, &ulBytes, ulOffsetSave, &ulActualBytesRead);
...
USHORT	TraceRecHash(USHORT hf, UCHAR *pucBuffer, PKEYINF pKey, PFILEHDR pHeader,
					PSEQMNP pSequence)
{
	BOOL	fSearchOriginal, fBreakRead;
	UCHAR	aucRecord[ENOUGHREC], fchFlags;
	USHORT	usStat;
	ULONG	ulCell, ulFrom;
	PRECCNT	pChain;
...
USHORT	TraceRecRelative(USHORT hf, UCHAR *pucBuffer, PKEYINF pKey, PFILEHDR pHeader,
					PSEQMNP pSequence)
{
	USHORT	usStat;
...
USHORT	TraceRecMainte(USHORT hf, UCHAR *pucBuffer, PKEYINF pKey, PFILEHDR pHeader,
					PSEQMNP pSequence)
{
	BOOL	fReadNext;
	UCHAR	aucRecord[ENOUGHREC];
...
USHORT	TraceRecIndex(USHORT hf, USHORT usManip, USHORT usFile, UCHAR *pucBuffer,
					PKEYINF pKey, PFILEHDR pHeader, PSEQMNP pSequence)
{
	...
	/* --- trace a record --- */
    /* --- read index file --- */
    if (usStat = PhyReadRecord(hf, ulCell, (UCHAR *)&RecIdx, pHeader)) {
    ...
    if (ADJUSTFILE(usFile) == FILE_PLU_INDEX) {
	    ...
	    usStat = MnpReadRecord(usFile, usManip, RecIdx.aucPluNumber, (USHORT)RecIdx.uchAdjective, pucBuffer);
    }
...
;========================================================================
**/
#if defined(EvaluateFileInfo)
USHORT	EvaluateFileInfo_Special(USHORT usFile, USHORT *pusHandle, PINFTBL *ppInfo);

USHORT	EvaluateFileInfo_Debug(USHORT usFile, USHORT *pusHandle, PINFTBL *ppInfo, char *aszFilePath, int nLineNo)
{
	int iLen = 0;
	char  xBuffer[256];
	...
	sprintf (xBuffer, "  EvaluateFileInfo_Debug(): hf = %d, File %s, lineno = %d", usFile, aszFilePath + iLen, nLineNo);
	NHPOS_ASSERT_TEXT(0, xBuffer);
	return EvaluateFileInfo_Special(usFile, pusHandle, ppInfo);
}
...
#endif
{
	...
	*ppInfo    = pTable;
	...
	usStat = *pusHandle ? SPLU_COMPLETED : SPLU_FILE_NOT_FOUND;	/* evaluate handle		*/
...
;========================================================================
**/
#if defined(SearchFileInfo)
PINFTBL	SearchFileInfo_Special(USHORT usFile);

PINFTBL	SearchFileInfo_Debug(USHORT usFile, char *aszFilePath, int nLineNo)
{
	int iLen = 0;
	char  xBuffer[256];
	PINFTBL		pTable;
	...
	sprintf (xBuffer, "  SearchFileInfo_Debug(): usFile = %d, File %s, lineno = %d", usFile, aszFilePath + iLen, nLineNo);
	NHPOS_ASSERT_TEXT(0, xBuffer);
	pTable = SearchFileInfo_Special(usFile);

	if (pTable) {
		USHORT	*phPluFile = &hPluFile;					/* handle of PLU file			*/
		USHORT	*phMixFile = &hMixFile;					/* handle of mix-match file		*/
		USHORT	*phPluMaint = &hPluMaint;					/* handle of PLU mainte. file	*/
		USHORT	*phPluIndex = &hPluIndex;					/* handle of PLU index file,R20	*/

		sprintf(xBuffer, "      usFileNo %d, pusHandle %p, usFileType %d 0x%x", pTable->usFileNo, pTable->pusHandle, pTable->usFileType, pTable->usFileType);
		NHPOS_ASSERT_TEXT(0, xBuffer);
		sprintf(xBuffer, "      *pusHandle = %d, %p, %p, %p, %p", *(pTable->pusHandle), phPluFile, phMixFile, phPluMaint, phPluIndex);
		NHPOS_ASSERT_TEXT(0, xBuffer);
	} else {
		NHPOS_NONASSERT_TEXT("    pTable = NULL");
	}
	return pTable;
}
...
{
	USHORT		i;
	PSEQMNP		pManip = aManipBlock;						/* top of control buffer	*/
...
ULONG	ComputeHash(TCHAR *pucKey, USHORT usLen, ULONG ulBase)
{
	...
	_asm {
		...
		push	ebx							; save registers ### MOD 2172 Rel1.0
		push	ecx							;
		push	edx							;
		push	esi							;
		push	edi							;
		...
		;--- initialize ---
		xor		eax, eax					; ### ADD 2172 Rel1.0
		xor		ecx, ecx					;
		xor		edx, edx					;

		cld								; reset F(D)=0, forward direction
		xor		ebx, ebx					; ### MOD 2172 Rel1.0
		xor		edi, edi					; (di:bx) = work reg.
		...
		mov		esi,pucKey					; ### MOD 2172 Rel1.0
		inc		esi							; advance data ptr. by 1
...
ComputeH02:
		mov		esi, pucKey			; (ds:si) = pucKey ### MOD 2172 Rel1.0
		mov		cx, usLen					; (cx) = no. of bytes
		shr		cx, 1						; (cx) = no. of words
...
SHORT	ComputeIndex(UCHAR *pucKey, USHORT usHandle, PFILEHDR pHeader,
                        ULONG *pulOffset, UCHAR *auchBuffer)
{
...
#else
        d13ProdWork = ulCenterBlock * RECIDX_BUFFER;
        d13ProdWork *= sizeof(RECIDX);
        *pulOffset += (ULONG)d13ProdWork;
#endif
		...
		ulBytes = (ulBufferRec * sizeof(RECIDX));			/* ### MOD 2172 Rel1.0 (USHORT->ULONG) */
		_PLU_ASSERT("ComputeIndex",NULL,(65535 >= (ulBufferRec * sizeof(RECIDX))));	/* ### ADD 2172 Rel1.0 */

		if ((SPLU_COMPLETED == PhyReadFile(usHandle, auchBuffer, &ulBytes, *pulOffset, &ulActualBytesRead)) 
            && (ulBytes)) {
...
/* {	FILE_PPI,		TYPE_RELATIVE,	&hPpiFile,	szPpiFile,	PpiFileKey		},*//* ### DEL 2172 Rel1.0 */
 ...
 {	TBLEND,			TBLEND,	(USHORT *)NULL,	(TCHAR *)NULL,	(PKEYINF)NULL	},
};
...
RECDIC	FARCONST	PluRecord[] = {
 ...
 { PLUID_FAMILY_CODE,	TYPUSH,
	FAROF(usFamilyCode,		 RECPLU), LONGOF(usFamilyCode,	    RECPLU)	},	/* ### ADD Saratoga (05172000)*/
