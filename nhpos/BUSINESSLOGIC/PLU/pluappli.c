...
USHORT	GetBatchNumber(USHORT, UCHAR *);
BOOL	IsActiveRecord(USHORT, UCHAR *);
BOOL	IsSatisfiedRecord(UCHAR *, UCHAR *, PRECDIC);
...
BOOL	IsValidCondition(UCHAR *, USHORT, PRECDIC);
...
USHORT	ExeOpenMirror(TCHAR FAR *pszFileName, TCHAR FAR *pszIndexName, USHORT *pusMirrorFile)
{
	USHORT	usStat;
	...
	*pusHandle = 0;								/* assume no handle respond	*/
	...
	if ((usFunc == FUNC_REPORT_COND) || (usFunc == FUNC_REPORT_MAINTE) ||
		...
		case FUNC_REPORT_COND2:					/* report rec. w/ cond. ?	*/
			...
			NHPOS_ASSERT_TEXT((usBytes <= sizeof(((PECMCOND)pvParam)->aucCond)), "**ERROR: ExeEnterCritMode() ((PECMCOND)pvParam)->aucCond size too small.");
			break;
		case FUNC_REPORT_MAINTE:				/* report mainte. rec. ?	*/
		default:								/* others ...				*/
			pucData = ((PECMMAINTE)pvParam)->aucCond;
			usBytes = ((PECMMAINTE)pvParam)->usLength;
			NHPOS_ASSERT_TEXT((usBytes <= sizeof(((PECMMAINTE)pvParam)->aucCond)), "**ERROR: ExeEnterCritMode() ((PECMMAINTE)pvParam)->aucCond size too small.");
			break;
		}
	...
	case FUNC_REPORT_RANGE2:  					/* report rec. by range ?	*/
		...
		*pucData++      = COMP_LESS_EQUAL | LOGIC_END;
		...
		NHPOS_ASSERT_TEXT((pUser->usLength <= sizeof(pUser->aucFuncData)), "**WARNING: FUNC_REPORT_RANGE pUser->aucFuncData size exceeded.");
	...
	case FUNC_REPORT_ADJECT:					/* report adjective rec. ?	*/
		pucData    = pUser->aucFuncData;			/* make option data		*/
		*pucData++ = uchItem;						/* make end item no.	*/
		memcpy(pucData, &(((PECMADJECT)pvParam)->itemPluNumber), pTable->usKeyLen);
		pucData        += pTable->usKeyLen;
		*pucData++      = COMP_EQUAL | LOGIC_END;
		pUser->usLength = (USHORT)(pucData - pUser->aucFuncData);
		NHPOS_ASSERT_TEXT((pUser->usLength <= sizeof(pUser->aucFuncData)), "**WARNING: FUNC_REPORT_ADJECT pUser->aucFuncData size exceeded.");
	...
	case FUNC_REPORT_COND2:						/* report rec. w/ cond. ?	*/
		...
		NHPOS_ASSERT_TEXT((pUser->usLength <= sizeof(pUser->aucFuncData)), "**WARNING: FUNC_REPORT_COND pUser->aucFuncData size exceeded.");
	...
	case FUNC_REPORT_MAINTE:					/* report mainte. rec. ?	*/
		...
		pUser->usLength = usBytes;
		NHPOS_ASSERT_TEXT((pUser->usLength <= sizeof(pUser->aucFuncData)), "**WARNING: FUNC_REPORT_MAINTE pUser->aucFuncData size exceeded.");
		memcpy(pUser->aucFuncData, pucData, usBytes);
		break;
	case FUNC_APPLY_MAINTE:						/* apply mainte. rec. ?		*/
	...
	case FUNC_REPORT_ADJECT:					/* report adjective rec. ?	*/
		...
		break;
	case FUNC_BLOCK_ACCESS:						/* block read / write ?		*/
		pUser->fchControl |= (UCHAR)((pUser->fsOption & BLOCK_READ) ? (USRBLK_PROH_WRITE)                    : 0);
		pUser->fchControl |= (UCHAR)((pUser->fsOption & BLOCK_WRITE) ? (USRBLK_PROH_WRITE + USRBLK_PROH_READ) : 0);
		break;
	case FUNC_BOUND_MAINTE:						/* associated mainte. ?		*/
                     ...
                     USHORT *pusRet)
{
	...
	*pusRet = 0;								/* assume no data returned	*/
	...
	switch (ADJUSTFILE(usFile)) {				/* what kind of file ?		*/
	case FILE_PLU:								/* PLU file ?				*/
		pucKey = (TCHAR*)((PRECPLU)pvRec)->aucPluNumber;
	...
	case FILE_PLU_INDEX:							/* PLU index file ?		*/
		pucKey = (TCHAR*)((PRECIDX)pvRec)->aucPluNumber;
    ...
    if (ADJUSTFILE(usFile) == FILE_PLU_INDEX) {
        ...
        for (i=0; i<MAX_DEPT; i++) {
    	    ...
    	    usStat = MnpReadRecord(usFile, HMNP_REC_MAIN, (UCHAR *)&RecIdx, 0, NULL);
						...
						USHORT *pusRet)
{
	...
	TCHAR		*pucKey;
	UCHAR		*pucStatus, aucRecord[ENOUGHREC];
	USHORT		usStat, usAdj;
	PKEYINF		pTable;
	PRECDIC		pRecord;
    RECIDX		RecIdx;
...
/*	} ### DEL 2172 Rel1.0 (01/07/00) */
    ...
    if ((ADJUSTFILE(usFile) == FILE_PLU_INDEX)) {
        ...
        usStat = MnpDeleteRecord(usFile, (UCHAR *)&RecIdx, usAdj);    /* delete index file*/
						...
						USHORT  *pusRet)
{
    ...
    if ((!usStat) && (ADJUSTFILE(usFile) == FILE_PLU)) {
      ...
      /*RecIdx.usDeptNumber = usPreDept;*/	/* ### DEL 2172 Rel1.0 (01/07/00) */
        _tcsncpy(RecIdx.aucPluNumber, pItemNumber, PLU_NUMBER_LEN);
        //memcpy(&RecIdx.aucPluNumber, pItemNumber, PLU_NUMBER_LEN); /* ### MOD 2172 Rel1.0 */
						...
						USHORT	*pusRet)
{
	...
	*pusRet = 0;								/* assume no data returned	*/
	...
	case FILE_PLU_INDEX:							/* PLU index file ?		*/
		pPlu     = (PRECPLU)NULL;						/* not PLU file			*/
		pucKey   = (/*BYTE*/UCHAR*)((PRECIDX)pItemNumber)->aucPluNumber;
...
/*			if (pPlu->lItemCounts || pPlu->lSalesTotal || pPlu->lSalesTotal) {  */
	...
	usStat = MnpDeleteRecord(usFile, (UCHAR *)pItemNumber, usAdj);
...
USHORT	ExeDeleteReserved(USHORT usHandle)
{
    ...
    if ((ADJUSTFILE(pUser->usFile) == FILE_PLU_INDEX)) {
        ...
        /* --- delete index record R2.0 --- */
     	...
     	_tcsncpy(RecIdx.aucPluNumber, pUser->itemNumber.aucPluNumber, PLU_NUMBER_LEN);
      ...
      /*RecIdx.usDeptNumber = pUser->usDeptNumber;*/	/* ### DEL 2172 Rel1.0 (01/07/00) */
    	...
    	usStat = MnpDeleteRecord(pUser->usFile, (UCHAR *)&RecIdx, 0);

    } else {

    	usStat = MnpDeleteRecord(pUser->usFile,			/* delete the record	*/
							 (UCHAR *)(&(pUser->itemNumber)),	/* item no.	*/
    ...
    if (usStat == SPLU_COMPLETED) {
          ...
          /*RecIdx.usDeptNumber = pUser->usDeptNumber;*/ /* ### DEL 2172 Rel1.0 (01/07/00) */
            ...
            usStat = MnpDeleteRecord(pUser->usIndexFile, (UCHAR *)&RecIdx, 0);   /* delete index file*/
						 ...
						 USHORT  *pusRet)
{
	...
	*pusRet = pTable->usBytesRec;				/* record length in byte	*/
	...
	_tcsncpy(pUser->itemNumber.aucPluNumber, pItemNumber->aucPluNumber, pTable->usKeyLen);
...
USHORT	ExeReadBlock(USHORT usHandle, UCHAR *pucBuffer, ULONG ulSize, ULONG *pulRead)
{
	USHORT		usStat;
	PUSRBLK		pUser;
	...
	*pulRead = 0;								/* assume no data returned	*/
	...
	usStat = MnpReadFile(pUser->hManip, pucBuffer, ulSize, pulRead);
...
USHORT	ExeWriteBlock(USHORT usHf, UCHAR *pucData, USHORT usBytes, USHORT *pusWritten)
{
	USHORT		usStat;
	PUSRBLK		pUser;
...
BOOL	IsValidCondition(UCHAR *pucCond, USHORT usBytes, PRECDIC pDictionary)
{
	BOOL		fEndOfData;
	UCHAR		*pucTrace, uchByte;
	SHORT		sRest;
	PRECDIC		pTable;
...
USHORT	GetBatchNumber(USHORT usFile, UCHAR *pucRecord)
{
	/* --- is PLU file ? --- */
...
BOOL	IsActiveRecord(USHORT usFile, UCHAR *pucRecord)
{
...
BOOL	IsSatisfiedRecord(UCHAR *pucRecord, UCHAR *pucCond, PRECDIC pDictionary)
{
	BOOL		fBreak, fSatisfied;
	UCHAR		*pucTrace, *pucValue, *pucData, uchByte;
	PRECDIC		pTable;
...
VOID SetIndexRecord(RECIDX *pRecIdx, RECPLU *pRecPlu)
{
    if (pRecPlu) {
        ...
        _tcsncpy(pRecIdx->aucPluNumber, pRecPlu->aucPluNumber, PLU_NUMBER_LEN);
        //memcpy(pRecIdx->aucPluNumber, pRecPlu->aucPluNumber, PLU_NUMBER_LEN); /* ### 2172 Rel1.0 */
