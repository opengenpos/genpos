...
SHORT   STUBENTRY SerOpDeptPluRead(PLUIF_DEP FAR *pPdp, USHORT usLockHnd)
{
    ...
    CliMsg.usReqLen     = sizeof(PLUIF_DEP);
...
SHORT   STUBENTRY CliOpMnemonicsIndRead(MLDIF *pMld, USHORT usLockHnd)
{
    CLIREQOPPARA    ReqMsgH;
    CLIRESOPPARA    ResMsgH;
	SHORT			sRetCode;
    ...
    memset(&CliMsg, 0, sizeof(CLICOMIF));
    CliMsg.usFunCode    = CLI_FCOPRSNINDREAD;
    CliMsg.pReqMsgH     = (CLIREQCOM *)&ReqMsgH;
    CliMsg.usReqMsgHLen = sizeof(CLIREQOPPARA);
    CliMsg.pResMsgH     = (CLIRESCOM *)&ResMsgH;
    CliMsg.usResMsgHLen = sizeof(CLIRESOPPARA);
...
SHORT	STUBENTRY SerOpMnemonicsAssign(MLDIF *pMld, USHORT usLockHnd)
{
    CLIREQOPPARA    ReqMsgH;
    CLIRESOPPARA    ResMsgH;
	SHORT			sRetCode;
    ...
    memset(&CliMsg, 0, sizeof(CLICOMIF));
    CliMsg.usFunCode    = CLI_FCOPRSNASSIGN;
    CliMsg.pReqMsgH     = (CLIREQCOM *)&ReqMsgH;
    CliMsg.usReqMsgHLen = sizeof(CLIREQOPPARA);
    CliMsg.pResMsgH     = (CLIRESCOM *)&ResMsgH;
    CliMsg.usResMsgHLen = sizeof(CLIRESOPPARA);
...
SHORT	STUBENTRY SerOpDisplayOperatorMessage (UCHAR *pucFlags, WCHAR *pTextString, ULONG ulTextCharsCount)
{
    CLIREQMSGDISP   ReqMsgH;
    CLIRESMSGDISP   ResMsgH;
	SHORT			sRetCode;

    PifRequestSem(husCliExeNet);
    memset(&ReqMsgH, 0, sizeof(ReqMsgH));
    memset(&ResMsgH, 0, sizeof(ResMsgH));

	memcpy (ReqMsgH.ucOperationalFlags, pucFlags, sizeof(ReqMsgH.ucOperationalFlags));
	ReqMsgH.usMsgLen = ulTextCharsCount;
	memcpy (ReqMsgH.usTextMessage, pTextString, ReqMsgH.usMsgLen * sizeof(pTextString[0]));

    memset(&CliMsg, 0, sizeof(CLICOMIF));
    CliMsg.usFunCode    = CLI_FCOPREQDISPMSG;
    CliMsg.pReqMsgH     = (CLIREQCOM *)&ReqMsgH;
    CliMsg.usReqMsgHLen = sizeof(CLIREQMSGDISP);
    CliMsg.pResMsgH     = (CLIRESCOM *)&ResMsgH;
    CliMsg.usResMsgHLen = sizeof(CLIRESMSGDISP);
...
SHORT	STUBENTRY SerOpRetrieveSystemStatus (USHORT  *usSystemStatus)
{
	...
	*usSystemStatus = ResMsgH.fsComStatus;
...
SHORT	STUBENTRY   SerOpUpdateExistingPlu (PLUIF *pPif, USHORT usLockHnd)
{
    ...
    memset(&CliMsg, 0, sizeof(CLICOMIF));
    CliMsg.usFunCode    = CLI_FCOPPLUUPDATE;
    CliMsg.pReqMsgH     = (CLIREQCOM *)&ReqMsgH;
    CliMsg.usReqMsgHLen = sizeof(CLIREQOPPARA);
    CliMsg.pResMsgH     = (CLIRESCOM *)&ResMsgH;
    CliMsg.usResMsgHLen = sizeof(CLIRESOPPARA);
