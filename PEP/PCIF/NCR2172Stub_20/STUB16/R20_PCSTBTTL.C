...
SHORT   STUBENTRY SerTtlTumUpdateDelayedBalance(VOID)
{
    ...
    memset(&CliMsg, 0, sizeof(CLICOMIF));
    CliMsg.usFunCode    = CLI_FCTTLDELAYBALUPD;
    CliMsg.pReqMsgH     = (CLIREQCOM *)&ReqMsgH;
    CliMsg.usReqMsgHLen = sizeof(CLIREQRESETTL);
    CliMsg.pResMsgH     = (CLIRESCOM *)&ResMsgH;
    CliMsg.usResMsgHLen = sizeof(CLIRESTOTAL);
...
*===========================================================================
*/
SHORT   STUBENTRY SerTtlTotalReset(VOID  *pTotal, USHORT usMDCBit)
{
    CLIREQRESETTL  	ReqMsgH;
    CLIRESTOTAL  	ResMsgH;
    TTLCASHIER FAR *pTtl;
...
*===========================================================================
*/
SHORT   STUBENTRY SerTtlIssuedReset(VOID  *pTotal, UCHAR fIssued)
{
    CLIREQRESETTL  	ReqMsgH;
    CLIRESTOTAL  	ResMsgH;
    TTLCASHIER      *pTtl;
...
*===========================================================================
*/
SHORT   STUBENTRY SerTtlUpdateFile(ISPPICKUP  *pData , USHORT usDataLen)
{
    CLIREQTOTAL     ReqMsgH;
    CLIRESTOTAL     ResMsgH;
    SHORT			sRetCode;
    ...
    ReqMsgH.usTransNo   = CliNetConfig.usCliTranNo ;
    ...
    if ( TTL_SUCCESS ==  CliMsg.sRetCode ) {
        CliNetConfig.usCliTranNo++;                       /* increment counter */
    }
    PifReleaseSem(husCliExeNet);
    return sRetCode;
}
...
VOID STUBENTRY SerTtlCloseSavFile()
{
	...
	memset(&CliMsg, 0, sizeof(CLICOMIF));
	CliMsg.usFunCode = CLI_FCTTLCLOSESAVTTL;
	CliMsg.pReqMsgH     = (CLIREQCOM *)&ReqMsgH;
	CliMsg.usReqMsgHLen = sizeof(CLIREQTOTAL);
    CliMsg.pResMsgH     = (CLIRESCOM *)&ResMsgH;
    CliMsg.usResMsgHLen = sizeof(CLIRESTOTAL);
	CliMsg.pauchReqData = &c;
    CliMsg.usReqLen     = sizeof(CLIREQRESETTL)-(2+PIF_LEN_IP);
	CstSendMaster();
	PifReleaseSem(husCliExeNet);
}
