...
******************************************************************************
fhfh*/
SHORT STUBENTRY RmtDeleteFile(LPCWSTR paszFileName)
{
    RMTREQDELETE    ReqMsg;
    RMTRESDELETE    ResMsg;
	SHORT			sRetCode;
    ...
    /* set the request and response message */
    memset(&CliMsg, 0, sizeof(CLICOMIF));
    wcsncpy(ReqMsg.aszFileName, paszFileName, RMT_FILENAME_SIZE);
...
******************************************************************************
**                                                                          **
**  Synopsis:   SHORT STUBENTRY RmtGetFileNames(LPCWSTR paszFileNames) **
**              paszFileName:   the pointer of file name                    **
**                                                                          **
**  Return:     see the function of "RmtErrCtl()"                           **
**                                                                          **
...
//	strncpy(ReqMsg.aszMode, paszMode, RMT_MODE_SIZE);
	{
		// function RmtFindDebugFileNames() in GenPOS which handles this request expects the
		// file mode to be a char array not a wchar_t array.
		SHORT i;
		for (i = 0; i < RMT_MODE_SIZE; i++) ReqMsg.aszMode[i] = paszMode[i];
	}
	wcsncpy(ReqMsg.aszExtension, paszExtension, (sizeof(ReqMsg.aszExtension)/2));
    CliMsg.pReqMsgH     = (CLIREQCOM *)&ReqMsg;
    CliMsg.usReqMsgHLen = sizeof(RMTREQNAME);
    CliMsg.pResMsgH     = (CLIRESCOM *)&ResMsg;
    CliMsg.usResMsgHLen = sizeof(RMTRESNAME);
	...
	if(sRetCode)
	{
		memcpy(paszFileNames, ResMsg.aszFileName, RMT_FILENAME_SIZE  * sizeof(WCHAR));
	}
...
******************************************************************************
**                                                                          **
**  Synopsis:   SHORT STUBENTRY RmtGetSavFileNames(WCHAR *paszFileNames)	**
**              paszFileName:   the pointer to array of file name           **
**                                                                          **
**  Return:     number of file names in array		                        **
**                                                                          **
** Description: This function requests .sav file names from terminal        **
**                                                                          **
******************************************************************************
fhfh*/
SHORT STUBENTRY RmtGetSavFileNames(WCHAR *paszFileNames)
{
    RMTREQSAVNAME   ReqMsg;
    RMTRESSAVNAME   ResMsg;
	SHORT			sRetCode;
    ...
    /* set the request and response message */
    memset(&CliMsg, 0, sizeof(CLICOMIF));
    CliMsg.usFunCode    = CLI_FCRMTSAVFILENM;
    CliMsg.pReqMsgH     = (CLIREQCOM *)&ReqMsg;
    CliMsg.usReqMsgHLen = sizeof(RMTREQSAVNAME);
    CliMsg.pResMsgH     = (CLIRESCOM *)&ResMsg;
    CliMsg.usResMsgHLen = sizeof(RMTRESSAVNAME);
	...
	if(sRetCode)
	{
		memcpy(paszFileNames, ResMsg.aszFileName, RMT_NUM_SAV_FILE_NM * RMT_FILENAME_SIZE * sizeof(WCHAR));
	}
...
******************************************************************************
fhfh*/
SHORT STUBENTRY RmtOpenFile(LPCWSTR paszFileName,
                            CONST CHAR FAR *paszMode)
{
    RMTREQOPEN      ReqMsg;
    RMTRESOPEN      ResMsg;
...
SHORT STUBENTRY RmtReadFile(USHORT usFile, VOID FAR *pBuffer, USHORT usBytes)
{
    ...
    while (sBytes > 0) {
        ...
        /* error ocuurs */
        if ((sResult = (SHORT)RmtErrCtl(CliMsg.sRetCode, (SHORT)ResMsg.sReturn)) < 0) {
		    PifReleaseSem(husCliExeNet);
            return(sResult);
        }
