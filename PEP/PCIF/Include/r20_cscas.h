...
*   Category           : Client/Server CASHIER module, US HOSPITALITY APPLICATION
...
SHORT   CasClose(USHORT usCashierNo);
SHORT   CasAssign(CASIF *Casif);
SHORT   CasDelete(USHORT usCashierNo);
SHORT   CasSecretClr(USHORT usCashierNo);
SHORT   CasAllIdRead(USHORT usRcvBufferSize,USHORT *ausRcvBuffer);
SHORT   CasIndRead(CASIF *Casif);
SHORT   CasIndLock(USHORT usCashierNo);
SHORT   CasIndUnLock(USHORT usCashierNo);
...
SHORT   CasChkSignIn(USHORT *ausRcvBuffer);
