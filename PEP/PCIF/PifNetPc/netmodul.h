...
# define    NET_TALLY                           /* enable/disable tally     */
//define    NET_DEBUG                           /* enable/disable debugging */
#elif       defined (POS7450)                   /* for 7450 POS model ?         */
# define    NET_TALLY                           /* enable/disable tally     */
# define    NET_DEBUG                           /* enable/disable debugging */
#elif   defined (POS2170)                   /* for 2170 POS model ?         */
# define    NET_TALLY                           /* enable/disable tally     */
//define    NET_DEBUG                           /* enable/disable debugging */
#elif   defined (SERVER_NT)                 /* for NT Server model ?        */
# define    NET_TALLY                           /* enable/disable tally     */
//define    NET_DEBUG                           /* enable/disable debugging */
#endif
...
#elif   defined(SARATOGA_PC)
# define    PRIO_NET_MGR    THREAD_PRIORITY_TIME_CRITICAL
# define    PRIO_NET_ECHO   THREAD_PRIORITY_TIME_CRITICAL
#elif       defined (POS7450)                   /* for 7450 POS model ?         */
# define    PRIO_NET_MGR    MAKEULONG(PRTYD_MAXIMUM, PRTYC_NOCHANGE)
# define    PRIO_NET_ECHO   MAKEULONG(PRTYD_MAXIMUM, PRTYC_NOCHANGE)
#elif   defined (POS2170)                   /* for 2170 POS model ?         */
//define    PRIO_NET_MGR    ?                   /* defined in TERMCFG.H     */
//define    PRIO_NET_ECHO   ?                   /* defined in TERMCFG.H     */
#elif   defined (SERVER_NT)                 /* for NT Server model ?        */
# define    PRIO_NET_MGR    THREAD_PRIORITY_TIME_CRITICAL
# define    PRIO_NET_ECHO   THREAD_PRIORITY_TIME_CRITICAL
#endif
...
# define    THREAD      FAR
 typedef VOID (THREAD *PFNTHREAD)(PVOID);
#elif   defined (POS2170)                   /* for 2170 POS model ?         */
# define    THREAD      _cdecl FAR _loadds
 typedef VOID (THREAD *PFNTHREAD)(PVOID);
#elif   defined (SERVER_NT)                 /* for NT Server model ?        */
# define    THREAD      FAR
 typedef VOID (THREAD *PFNTHREAD)(PVOID);
...
# define    WAIT_FOREVER    INFINITE            /* wait forever             */
  ...
  typedef struct _SEMOBJ {                      /* semaphore object         */
    HANDLE  hSemaphore;                             /* handle of semaphore  */
  } SEMOBJ, FAR *PSEMOBJ;
...
# define    WAIT_FOREVER    INFINITE            /* wait forever             */
  ...
  typedef struct _SEMOBJ {                      /* semaphore object         */
    ULONG   ulSemaphore;                            /* RAM semaphore itself */
  } SEMOBJ, FAR *PSEMOBJ;

  typedef struct _EVTOBJ {                      /* event signal object      */
    ULONG   ulEvent;                                /* RAM semaphore itself */
  } EVTOBJ, FAR *PEVTOBJ;

# define    WAIT_FOREVER    SEM_INDEFINITE_WAIT /* wait forever             */
  ...
  typedef struct _EVTOBJ {                      /* event signal object      */
    USHORT  usEvent;                                /* handle of event      */
  } EVTOBJ, FAR *PEVTOBJ;

# define    WAIT_FOREVER    0L                  /* wait forever             */
...
#else                                       /* unknow target module ...     */
# error Target module name is not defined !
#endif
...
#if     defined(SARATOGA)
# define    MEM_HEAP_BY_WIN32                   /* use WIN32 heap           */
#elif   defined(SARATOGA_PC)
# define    MEM_INTERNAL_LOGIC                  /* use internal logic       */
# define    MEM_TOTAL_BYTES     (7 * KBYTES)    /* total memory capacity    */
//# define    MEM_HEAP_BY_WIN32                   /* use WIN32 heap           */
#elif       defined (POS7450)                   /* for 7450 POS model ?         */
# define    MEM_INTERNAL_LOGIC                  /* use internal logic       */
# define    MEM_TOTAL_BYTES     (7 * KBYTES)    /* total memory capacity    */
//define    MEM_HEAP_BY_BMALLOC                 /* use bmalloc logic        */
#elif   defined (POS2170)                   /* for 2170 POS model ?         */
# define    MEM_INTERNAL_LOGIC                  /* use internal logic       */
# define    MEM_TOTAL_BYTES     (3 * KBYTES)    /* total memory capacity    */
#elif   defined (SERVER_NT)                 /* for NT Server model ?        */
# define    MEM_HEAP_BY_WIN32                   /* use WIN32 heap           */
#endif
...
#if     defined (MEM_INTERNAL_LOGIC)        /* internal logic ?             */
 typedef    USHORT              HMEM;           /* memory handle            */
#elif   defined (MEM_HEAP_BY_BMALLOC)       /* heap using bmalloc() ?       */
 typedef    _segment            HMEM;           /* memory handle            */
#elif   defined (MEM_HEAP_BY_WIN32)         /* heap by WIN32 ?              */
...
#define ulOpenFaddr     datOption.datOpen.ulFaddr       /*                  */
#define usOpenFport     datOption.datOpen.usFport       /*                  */
#define usOpenLport     datOption.datOpen.usLport       /*                  */
#define usRecvBytes     datOption.datReceive.usBytes    /*                  */
#define pvRecvBuffer    datOption.datReceive.usBytes    /*                  */
#define usSendBytes     datOption.datSend.usBytes       /*                  */
#define pvSendBuffer    datOption.datSend.usBytes       /*                  */
#endif
...
typedef struct _RCVMSG {                        /* receiver I/F message     */
    ...
    union {                                         /* optional data        */
        SHORT   iSocket;                                /* new socket handle*/
...
/* -------------------------------- *\
 + C  R u n  T i m e  L i b r a r y +
\* -------------------------------- */
#if defined(SARATOGA)
# define    _fmemset(x, y, z)       memset(x, y, z) /* far memory set       */
# define    _fmemcpy(x, y, z)       memcpy(x, y, z) /* far memory copy      */
#elif defined(SARATOGA_PC)
# define    _fmemset(x, y, z)       memset(x, y, z) /* far memory set       */
# define    _fmemcpy(x, y, z)       memcpy(x, y, z) /* far memory copy      */
...
SHORT NETENTRY NetSend(USHORT, LPVOID, USHORT);
...
SHORT   NetHandler(PNETARG);
USHORT  ExeNetOpenCli(POPNDAT);
USHORT  ExeNetReceiveCli(USHORT, PRCVDAT);
USHORT  ExeNetSendCli(USHORT, PSNDDAT);
USHORT  ExeNetControlCli(USHORT, PCNTDAT);
USHORT  ExeNetCloseCli(USHORT);
USHORT  ExeNetShutdownCli(USHORT);
...
HMEM    SysCreateMemory(USHORT);                /* create memory segment    */
