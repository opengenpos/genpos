...
/*
* ===========================================================================
**  Synopsis:  BOOL  FileMigrateGP_205_220_Para(UNINIRAM_HGV_2_0_5 *pOldPara, UNINIRAM *pNewPara)
*
*   Input   :   UNINIRAM_HGV_2_0_5  * pOldPara   - Rel 2.0.5 type of Param.
**
*   Output  :   UNINIRAM           * pNewPara   - Rel 2.2.0 type of Param.
**
...
*                 from Release 2.0.5 to Release 2.2.0.
*				  The conversion is done from the provided UNINIRAM buffer
*                 for Release 2.0.5 to a buffer for Release 2.2.0.
*
* ===========================================================================
*/
BOOL  FileMigrateGP_205_220_Para (UNINIRAM_HGV_2_0_5 *pOldPara, UNINIRAM_HGV_2_2_0 *pNewPara)
{
    USHORT     j,i;
	LONG	   fileOffset = 0;
	...
	memset (pNewPara, 0, sizeof(UNINIRAM_HGV_2_2_0));
...
/*
* ===========================================================================
**  Synopsis:  BOOL  FileMigrateGP_210_220_Para(UNINIRAM_HGV_2_1_0 *pOldPara, UNINIRAM *pNewPara)
*
...
*
**  Description:  This function converts the parameter data in a buffer
*                 from Release 2.1.0 to Release 2.2.0, the current release.
*                 The conversion is done from the provided UNINIRAM buffer
*                 for Release 2.1.0 to a buffer for Release 2.2.0.
*
* ===========================================================================
*/
...
*/
	...
	//AC #7
	//job ranges new to version 2.2.0
	//and the area is already set to 0 thanks to the memset at the top of this function
...
/*
* ===========================================================================
**  Synopsis:  BOOL  FileMigrateGP_212_220_Para(UNINIRAM_HGV_2_1_2 *pOldPara, UNINIRAM *pNewPara)
*
*   Input   :   UNINIRAM_HGV_2_1_2  * pOldPara   - Rel 2.1.2 type of Param.
**
*   Output  :   UNINIRAM  * pNewPara   - Rel 2.2.0 type of Param.
**
*   Return  :   TRUE           - user process is executed
...
*
**  Description:  This function converts the parameter data in a buffer
*                 from Release 2.1.2 to Release 2.2.0, the current release.
*                 The conversion is done from the provided UNINIRAM buffer
*                 for Release 2.1.2 to a buffer for Release 2.2.0.
*
* ===========================================================================
*/
...
\
	...
	//Prog. #3, conversion neccessary.
	for( i = 0; i < MAX_FSC_TBL_HGV_2_1_2; i++)
	{
		memcpy(&pNewPara->ParaFSC[i], &pOldPara->ParaFSC[i], sizeof(pOldPara->ParaFSC[i]));
	}
	...
	*/
	...
	//Transaction Mnemonics Conversion
	for( i = 0; i < MAX_TRANSM_NO_HGV_2_1_2; i++){
		if( !wcsncmp(pOldPara->ParaTransMnemo[i], szNull, PARA_TRANSMNEMO_LEN_HGV_2_1_2) ||
			!wcsncmp(pOldPara->ParaTransMnemo[i], szSpaceTrans, PARA_TRANSMNEMO_LEN_HGV_2_1_2) )
		{/*  | Do Nothing |  */} else {
			wcsncpy(pNewPara->ParaTransMnemo[i], pOldPara->ParaTransMnemo[i],PARA_TRANSMNEMO_LEN_HGV_2_1_2);
		}
	}

	for( i = 0; i < MAX_LEAD_NO_HGV_2_1_2; i++) {
		if( !wcsncmp(pOldPara->ParaLeadThru[i], szNull, PARA_LEADTHRU_LEN_HGV_2_1_2) ||
			!wcsncmp(pOldPara->ParaLeadThru[i], szSpaceLDT, PARA_LEADTHRU_LEN_HGV_2_1_2) )
		{ /* Do Nothing */} else {
			wcsncpy(pNewPara->ParaLeadThru[i],pOldPara->ParaLeadThru[i],PARA_LEADTHRU_LEN_HGV_2_1_2);
		}
	}

	//Report Name Mnemonics Conversion
	for( i = 0; i < MAX_REPO_NO_HGV_2_1_2; i++) {
		if( !wcsncmp(pOldPara->ParaReportName[i], szNull, PARA_REPORTNAME_LEN_HGV_2_1_2) ||
			!wcsncmp(pOldPara->ParaReportName[i], szSpaceLDT, PARA_REPORTNAME_LEN_HGV_2_1_2) )
		{ /* Do Nothing */} else {
			wcsncpy(pNewPara->ParaReportName[i],pOldPara->ParaReportName[i],PARA_REPORTNAME_LEN_HGV_2_1_2);
		}
	}

	//Special Mnemonics Conversion
	for( i = 0; i < MAX_SPEM_NO_HGV_2_1_2; i++ ){
		if( !wcsncmp(pOldPara->ParaSpeMnemo[i], szNull, PARA_SPEMNEMO_LEN_HGV_2_1_2) ||
			!wcsncmp(pOldPara->ParaSpeMnemo[i], szSpaceSpec, PARA_SPEMNEMO_LEN_HGV_2_1_2) )
		{/* Do Nothing */} 	else {
			wcsncpy(pNewPara->ParaSpeMnemo[i], pOldPara->ParaSpeMnemo[i],PARA_SPEMNEMO_LEN_HGV_2_1_2);
		}
	}

	//Adjectives Mnemonics Conversion
	for( i = 0; i < MAX_ADJM_NO_HGV_2_1_2; i++ ){
		if( !wcsncmp(pOldPara->ParaAdjMnemo[i], szNull, PARA_ADJMNEMO_LEN_HGV_2_1_2) ||
			!wcsncmp(pOldPara->ParaAdjMnemo[i], szSpaceSpec, PARA_ADJMNEMO_LEN_HGV_2_1_2) )
		{/* Do Nothing */} 	else {
			wcsncpy(pNewPara->ParaAdjMnemo[i], pOldPara->ParaAdjMnemo[i],PARA_ADJMNEMO_LEN_HGV_2_1_2);
		}
	}
	...
	// Print Modifier Mnemonics Conversion
	for(i = 0; i < MAX_PRTMODI_NO_HGV_2_1_2; i++) {
		if( !wcsncmp(pOldPara->ParaPrtModi[i], szNull, PARA_PRTMODI_LEN_HGV_2_1_2) ||
			!wcsncmp(pOldPara->ParaPrtModi[i], szSpacePri, PARA_PRTMODI_LEN_HGV_2_1_2) )
		{ /* Do Nothing */} else {
			wcsncpy(pNewPara->ParaPrtModi[i],pOldPara->ParaPrtModi[i],PARA_PRTMODI_LEN_HGV_2_1_2);
		}
	}
	...
	//CPMDATA Prog. #54
	{
		...
		for (j = 0; j < PARA_TEND_CPM_NO_HGV_2_1_2; j++) {
			pNewPara->ParaHotelId.auchTend[j] = pOldPara->ParaHotelId.auchTend[j];
		}
		for (j = 0; j < sizeof(pOldPara->ParaHotelId.auchApproval)/sizeof(pOldPara->ParaHotelId.auchApproval[0]); j++) {
			pNewPara->ParaHotelId.auchApproval[j] = pOldPara->ParaHotelId.auchApproval[j];
...
*/
	...
	//A/C #116 Promotional PLU table
	for (i = 0; i < MAX_SETMENU_NO_HGV_2_1_2; i++) {
		memcpy(&pNewPara->ParaMenuPLU[i], &pOldPara->ParaMenuPLU[i], sizeof(pOldPara->ParaMenuPLU[i]));
	}
	...
	//Prog. 97 TERMINAL INFORMATION
	for( i = 0; i < MAX_TERMINALINFO_SIZE_HGV_2_1_2; i++) 
	{
		pNewPara->TerminalInformation[i].ulCodePage = pOldPara->TerminalInformation[i].ulCodePage;
		pNewPara->TerminalInformation[i].usLanguageID = pOldPara->TerminalInformation[i].usLanguageID;
		pNewPara->TerminalInformation[i].usSubLanguageID = pOldPara->TerminalInformation[i].usSubLanguageID;
...
/*
* ===========================================================================
**  Synopsis:  BOOL  FileMigrateGP_220_221_Para(UNINIRAM_HGV_2_2_0 *pOldPara, UNINIRAM *pNewPara)
*
*   Input   :   UNINIRAM_HGV_2_2_0  * pOldPara   - Rel 2.2.0 type of Param.
**
*   Output  :   UNINIRAM  * pNewPara   - Rel 2.2.1 type of Param.
**
*   Return  :   TRUE           - user process is executed
*               FALSE          - default process is expected
*
**  Description:  This function converts the parameter data in a buffer
*                 from Release 2.2.0 to Release 2.2.1, the current release.
*                 The conversion is done from the provided UNINIRAM buffer
*                 for Release 2.2.0 to a buffer for Release 2.2.1.
*
*                 There were the following changes made to UNINIRAM from 2.2.0 to 2.2.1
*                   - number of 24 Character Mnemonics (P57) increased from 20 to 40
*                   - number of Lead Thru Mnemonics (P21) increased from 180 to 220
*                   - number of Special Mnemonics (P23) increased from 50 to 75
*
* ===========================================================================
*/
...
\
	...
	//Prog. #3, conversion neccessary.
	for( i = 0; i < MAX_FSC_TBL_HGV_2_2_0; i++)
	{
		memcpy(&pNewPara->ParaFSC[i], &pOldPara->ParaFSC[i], sizeof(pOldPara->ParaFSC[i]));
	}
	...
	*/
	...
	//Transaction Mnemonics Conversion
	for( i = 0; i < MAX_TRANSM_NO_HGV_2_2_0; i++){
		if( !wcsncmp(pOldPara->ParaTransMnemo[i], szNull, PARA_TRANSMNEMO_LEN_HGV_2_2_0) ||
			!wcsncmp(pOldPara->ParaTransMnemo[i], szSpaceTrans, PARA_TRANSMNEMO_LEN_HGV_2_2_0) )
		{/*  | Do Nothing |  */} else {
			wcsncpy(pNewPara->ParaTransMnemo[i], pOldPara->ParaTransMnemo[i],PARA_TRANSMNEMO_LEN_HGV_2_2_0);
		}
	}

	//Lead Thru Mnemonics Conversion
	for( i = 0; i < MAX_LEAD_NO_HGV_2_2_0; i++) {
		if( !wcsncmp(pOldPara->ParaLeadThru[i], szNull, PARA_LEADTHRU_LEN_HGV_2_2_0) ||
			!wcsncmp(pOldPara->ParaLeadThru[i], szSpaceLDT, PARA_LEADTHRU_LEN_HGV_2_2_0) )
		{ /* Do Nothing */} else {
			wcsncpy(pNewPara->ParaLeadThru[i],pOldPara->ParaLeadThru[i],PARA_LEADTHRU_LEN_HGV_2_2_0);
		}
	}

	//Report Name Mnemonics Conversion
	for( i = 0; i < MAX_REPO_NO_HGV_2_2_0; i++) {
		if( !wcsncmp(pOldPara->ParaReportName[i], szNull, PARA_REPORTNAME_LEN_HGV_2_2_0) ||
			!wcsncmp(pOldPara->ParaReportName[i], szSpaceLDT, PARA_REPORTNAME_LEN_HGV_2_2_0) )
		{ /* Do Nothing */} else {
			wcsncpy(pNewPara->ParaReportName[i],pOldPara->ParaReportName[i],PARA_REPORTNAME_LEN_HGV_2_2_0);
		}
	}

	//Special Mnemonics Conversion
	for( i = 0; i < MAX_SPEM_NO_HGV_2_2_0; i++ ){
		if( !wcsncmp(pOldPara->ParaSpeMnemo[i], szNull, PARA_SPEMNEMO_LEN_HGV_2_2_0) ||
			!wcsncmp(pOldPara->ParaSpeMnemo[i], szSpaceSpec, PARA_SPEMNEMO_LEN_HGV_2_2_0) )
		{/* Do Nothing */} 	else {
			wcsncpy(pNewPara->ParaSpeMnemo[i], pOldPara->ParaSpeMnemo[i],PARA_SPEMNEMO_LEN_HGV_2_2_0);
		}
	}

	//Adjectives Mnemonics Conversion
	for( i = 0; i < MAX_ADJM_NO_HGV_2_2_0; i++ ){
		if( !wcsncmp(pOldPara->ParaAdjMnemo[i], szNull, PARA_ADJMNEMO_LEN_HGV_2_2_0) ||
			!wcsncmp(pOldPara->ParaAdjMnemo[i], szSpaceSpec, PARA_ADJMNEMO_LEN_HGV_2_2_0) )
		{/* Do Nothing */} 	else {
			wcsncpy(pNewPara->ParaAdjMnemo[i], pOldPara->ParaAdjMnemo[i],PARA_ADJMNEMO_LEN_HGV_2_2_0);
		}
	}
	...
	// Print Modifier Mnemonics Conversion
	for(i = 0; i < MAX_PRTMODI_NO_HGV_2_2_0; i++) {
		if( !wcsncmp(pOldPara->ParaPrtModi[i], szNull, PARA_PRTMODI_LEN_HGV_2_2_0) ||
			!wcsncmp(pOldPara->ParaPrtModi[i], szSpacePri, PARA_PRTMODI_LEN_HGV_2_2_0) )
		{ /* Do Nothing */} else {
			wcsncpy(pNewPara->ParaPrtModi[i],pOldPara->ParaPrtModi[i],PARA_PRTMODI_LEN_HGV_2_2_0);
		}
	}
	...
	//CPMDATA Prog. #54
	{
		...
		for (j = 0; j < PARA_TEND_CPM_NO_HGV_2_2_0; j++) {
			pNewPara->ParaHotelId.auchTend[j] = pOldPara->ParaHotelId.auchTend[j];
		}
		for (j = 0; j < sizeof(pOldPara->ParaHotelId.auchApproval)/sizeof(pOldPara->ParaHotelId.auchApproval[0]); j++) {
			pNewPara->ParaHotelId.auchApproval[j] = pOldPara->ParaHotelId.auchApproval[j];
	...
	//Prog. #57 24 Character Mnemonics Conversion
	for( i = 0; i < MAX_CH24_NO_HGV_2_2_0; i++ ){
		if( !wcsncmp(pOldPara->ParaChar24[i], szNull, PARA_CHAR24_LEN_HGV_2_2_0) ||
			!wcsncmp(pOldPara->ParaChar24[i], szSpaceSpec, PARA_CHAR24_LEN_HGV_2_2_0) )
		{/* Do Nothing */} 	else {
			wcsncpy(pNewPara->ParaChar24[i], pOldPara->ParaChar24[i],PARA_CHAR24_LEN_HGV_2_2_0);
		}
	}
...
*/
	...
	//A/C #116 Promotional PLU table
	for (i = 0; i < MAX_SETMENU_NO_HGV_2_2_0; i++) {
		memcpy(&pNewPara->ParaMenuPLU[i], &pOldPara->ParaMenuPLU[i], sizeof(pOldPara->ParaMenuPLU[i]));
	}
	...
	//A/C #87
//	memcpy(&pNewPara->ParaSoftChk, &pOldPara->ParaSoftChk, sizeof(pOldPara->ParaSoftChk));
	//Trailer EPT Logo Mnemonics Conversion
	for( i = 0; i < MAX_SOFTCHK_NO_HGV_2_2_0; i++) {
		if( !wcsncmp(pOldPara->ParaSoftChk[i], szNull, PARA_PROMOTION_LEN_HGV_2_2_0) ||
			!wcsncmp(pOldPara->ParaSoftChk[i], szSpaceLDT, PARA_PROMOTION_LEN_HGV_2_2_0) )
		{ /* Do Nothing */} else {
			wcsncpy(pNewPara->ParaSoftChk[i], pOldPara->ParaSoftChk[i], PARA_PROMOTION_LEN_HGV_2_2_0);
		}
	}
	...
	//Prog. 97 TERMINAL INFORMATION
	for( i = 0; i < MAX_TERMINALINFO_SIZE_HGV_2_2_0; i++) 
	{
		pNewPara->TerminalInformation[i].ulCodePage = pOldPara->TerminalInformation[i].ulCodePage;
		pNewPara->TerminalInformation[i].usLanguageID = pOldPara->TerminalInformation[i].usLanguageID;
		pNewPara->TerminalInformation[i].usSubLanguageID = pOldPara->TerminalInformation[i].usSubLanguageID;
...
//	FHEADER_HGV201203 fileHeader_GPV203;
	LONG	   fileOffset =0;
	...
	memset(pOldPara, 0, sizeof(UNINIRAM_HGV_2_1_0));

 //   /* move seek pointer    */
 //   if (SetFilePointer(iFile, 0L, NULL, FILE_BEGIN) != 0) {
 //       return TRUE;
 //   }
	...
	if(!GenFileReadAtOffset(iFile,0L,&Fheader,sizeof(Fheader))){	// read header from start of file
		return FALSE;
	}

#if 0
	if (fRet == FILE_HEAD_VER_GP203 || fRet == FILE_HEAD_VER_GP201 ||
		(fRet == FILE_HEAD_VER_GP204 && usFileVersionInfo[3] < 21) ) {
		/* read header  */
		ReadFile(iFile, &fileHeader_GPV203, sizeof(FHEADER_HGV201203), &bytesRead, NULL);
		if (bytesRead != sizeof(FHEADER_HGV201203)) {
			return FALSE;
		}
		lstrcpyW(Fheader.Fid.szClass, fileHeader_GPV203.Fid.szClass);
		lstrcpyW(Fheader.Fid.szModel, fileHeader_GPV203.Fid.szModel);
		lstrcpyW(Fheader.Fid.szName, fileHeader_GPV203.Fid.szName);
		lstrcpyW(Fheader.Fid.szRptName, fileHeader_GPV203.Fid.szRptName);
		lstrcpyW(Fheader.Fid.szTouchLayoutName, fileHeader_GPV203.Fid.szTouchLayoutName);
		lstrcpyW(Fheader.Fid.szVer, fileHeader_GPV203.Fid.szVer);

		for (iLoop = 0; iLoop < sizeof(fileHeader_GPV203.File.aRoom)/sizeof(fileHeader_GPV203.File.aRoom[0]); iLoop++) {
			Fheader.File.aRoom[iLoop] = fileHeader_GPV203.File.aRoom[iLoop];
		}
		Fheader.Mem = fileHeader_GPV203.Mem;
		memcpy (Fheader.szReceiptLogoName, fileHeader_GPV203.szReceiptLogoName, sizeof(fileHeader_GPV203.szReceiptLogoName));
	}
	else if (fRet == FILE_HEAD_VER_GP204) {
		/* In 2.0.4.21 and later the Fheader.File.aRoom array appears to be 1 element larger
		...
		for (iLoop = 0; iLoop < sizeof(Fheader.File.aRoom)/sizeof(Fheader.File.aRoom[0]); iLoop++) {
			Fheader.File.aRoom[iLoop] = fileHeader_GPV203.File.aRoom[iLoop];
		}
		Fheader.Mem = fileHeader_GPV203.Mem;
		memcpy (Fheader.szReceiptLogoName, fileHeader_GPV203.szReceiptLogoName, sizeof(fileHeader_GPV203.szReceiptLogoName));
	...
	/* divide each memory   */
	/* parameter            */
	//if(SetFilePointer(iFile, Fheader.Mem.Para.lOffset, NULL, FILE_BEGIN) == INVALID_SET_FILE_POINTER) {
	//	fileError = GetLastError();
	//	return FALSE;
	//}
	//ReadFile(iFile, pOldPara, Fheader.Mem.Para.lLen, &bytesRead, NULL);
	//if (bytesRead != (DWORD)Fheader.Mem.Para.lLen) {
	//	fileError = GetLastError();
	//	return FALSE;
	//}

	if(!GenFileReadAtOffset(iFile,Fheader.Mem.Para.lOffset, pOldPara, Fheader.Mem.Para.lLen)){
		return FALSE;
	}
...
BOOL  FileMigrateGP_220_Files (HANDLE iFile, UNINIRAM_HGV_2_2_0 *pOldPara, int fRet)
{
	int        iLoop;
    WCHAR      szFname[FILE_MAXLEN_TEXT];
	LONG	   fileOffset =0;
	...
	// for dividing up the files.
	LoadString(hPepInst, IDS_FILE_NAME_DEPT, szFname, FILE_MAXLEN_TEXT);
	FileToDepartmentFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_DEPT - IDS_FILE_NAME_DEPT].lOffset,
						 Fheader.File.aRoom[IDS_FILE_NAME_DEPT - IDS_FILE_NAME_DEPT].lLen,
						 szFname, fRet);
				
	LoadString(hResourceDll, IDS_FILE_NAME_PLUTMP, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToEachFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_PLU - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_PLU - IDS_FILE_NAME_DEPT].lLen,
					   szFname);

	LoadString(hResourceDll, IDS_FILE_NAME_SER, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToEachFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_SER - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_SER - IDS_FILE_NAME_DEPT].lLen,
					   szFname);

	//the index of the cashier file changed since version 210 so a new one is used here
	LoadString(hResourceDll, IDS_FILE_NAME_CAS, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToCashierFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_CAS - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_CAS - IDS_FILE_NAME_DEPT].lLen,
					   szFname, fRet);

	LoadString(hResourceDll, IDS_FILE_NAME_ETK, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToEmployeeTimeFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_ETK - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_ETK - IDS_FILE_NAME_DEPT].lLen,
					   szFname, fRet);

	LoadString(hResourceDll, IDS_FILE_NAME_DIRECT, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToEachFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_DIRECT - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_DIRECT - IDS_FILE_NAME_DEPT].lLen,
					   szFname);

	LoadString(hResourceDll, IDS_FILE_NAME_CPN, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToCouponFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_CPN - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_CPN - IDS_FILE_NAME_DEPT].lLen,
						szFname, fRet);

	LoadString(hResourceDll, IDS_FILE_NAME_CSTR, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToEachFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_CSTR - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_CSTR - IDS_FILE_NAME_DEPT].lLen,
					   szFname);

	LoadString(hResourceDll, IDS_FILE_NAME_PPI, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToPpiFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_PPI - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_PPI - IDS_FILE_NAME_DEPT].lLen,
						szFname, fRet);

	LoadString(hResourceDll, IDS_FILE_NAME_MLD, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToEachFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_MLD - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_MLD - IDS_FILE_NAME_DEPT].lLen,
					   szFname);

    iLoop = 15;
    LoadString(hResourceDll,  IDS_FILE_NAME_OEPFILE, szFname, PEP_STRING_LEN_MAC(szFname));
    FileToEachFile(iFile, Fheader.File.aRoom[iLoop].lOffset, Fheader.File.aRoom[iLoop].lLen, szFname);
	...
	//Added for R10 to R20, The Index is held in its own file now, so we must transfer it over
	// to the new file as well  JHHJ
	if(FileToEachFile(iFile, Fheader.File.aRoom[IDS_FILE_NAME_PLUINDEX - IDS_FILE_NAME_DEPT].lOffset,
				Fheader.File.aRoom[IDS_FILE_NAME_PLUINDEX - IDS_FILE_NAME_DEPT].lLen,
				WIDE("PARAMIDX2")) == FALSE){
		return TRUE;
	}
	...
	LoadString(hResourceDll, IDS_FILE_NAME_PLU, szFname, PEP_STRING_LEN_MAC(szFname));
	FileToPlu203File(iFile, Fheader.File.aRoom[IDS_FILE_NAME_PLU - IDS_FILE_NAME_DEPT].lOffset,
						Fheader.File.aRoom[IDS_FILE_NAME_PLU - IDS_FILE_NAME_DEPT].lLen,
						szFname);
