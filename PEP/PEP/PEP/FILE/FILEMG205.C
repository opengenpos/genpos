...
BOOL  FileMigrateGP_205_Files (HANDLE iFile, UNINIRAM_HGV_2_0_5 *pOldPara, int fRet)
{
	...
	memset(pOldPara, 0, sizeof(UNINIRAM_HGV_2_0_5));
	...
	if (fRet == FILE_HEAD_VER_GP205 || fRet == FILE_HEAD_VER_GP203 ||
		(fRet == FILE_HEAD_VER_GP204 && usFileVersionInfo[3] < 21) ) {
		/* read header  */
		FHEADER_HGV201203 fileHeader_GPV205 = { 0 };

		ReadFile(iFile, &fileHeader_GPV205, sizeof(FHEADER_HGV201203), &bytesRead, NULL);
		if (bytesRead != sizeof(FHEADER_HGV201203)) {
			return FALSE;
		}
		lstrcpyW(Fheader.Fid.szClass, fileHeader_GPV205.Fid.szClass);
		lstrcpyW(Fheader.Fid.szModel, fileHeader_GPV205.Fid.szModel);
		lstrcpyW(Fheader.Fid.szName, fileHeader_GPV205.Fid.szName);
		lstrcpyW(Fheader.Fid.szRptName, fileHeader_GPV205.Fid.szRptName);
		lstrcpyW(Fheader.Fid.szTouchLayoutName, fileHeader_GPV205.Fid.szTouchLayoutName);
		lstrcpyW(Fheader.Fid.szVer, fileHeader_GPV205.Fid.szVer);

		for (iLoop = 0; iLoop < sizeof(fileHeader_GPV205.File.aRoom)/sizeof(fileHeader_GPV205.File.aRoom[0]); iLoop++) {
			Fheader.File.aRoom[iLoop] = fileHeader_GPV205.File.aRoom[iLoop];
		}
		Fheader.Mem = fileHeader_GPV205.Mem;
		memcpy (Fheader.szReceiptLogoName, fileHeader_GPV205.szReceiptLogoName, sizeof(fileHeader_GPV205.szReceiptLogoName));
	}
...
/*
* ===========================================================================
**  Synopsis:  BOOL  FileMigrateGP_203_205_Para(UNINIRAM_HGV_2_0_3 *pOldPara, UNINIRAM_HGV_2_0_5 *pNewPara)
*
*   Input   :   UNINIRAM_HGV_2_0_3  * pOldPara   - Rel 2.0.3 type of Param.
**
*   Output  :   UNINIRAM_HGV_2_0_5  * pNewPara   - Rel 2.0.5 type of Param.
**
...
*                 from Release 2.0.3 to Release 2.0.5.
*				  The conversion is done from the provided UNINIRAM buffer
*                 for Release 2.0.3 to a buffer for Release 2.0.5.
*
* ===========================================================================
*/
BOOL  FileMigrateGP_203_205_Para (UNINIRAM_HGV_2_0_3 *pOldPara, UNINIRAM_HGV_2_0_5 *pNewPara)
{
    USHORT     j,i;
	LONG	   fileOffset = 0;
	...
	memset (pNewPara, 0, sizeof(UNINIRAM_HGV_2_0_5));
	...
	for( i = 0; i < MAX_FSC_TBL_HGV_2_0_3; i++)
	{
	...
	*/
	...
	for(i = 0; i <= MAX_PRTMODI_NO_HGV_2_0_3; i++) {
		if( !wcsncmp(pOldPara->ParaPrtModi[i], szNull, PARA_PRTMODI_LEN_HGV_2_0_3) ||
			!wcsncmp(pOldPara->ParaPrtModi[i], szSpacePri, PARA_PRTMODI_LEN_HGV_2_0_3) )
		{ /* Do Nothing */} else {
			wcsncpy(pNewPara->ParaPrtModi[i],pOldPara->ParaPrtModi[i],PARA_PRTMODI_LEN_HGV_2_0_3);
		}
	}
...
/*
* ===========================================================================
**  Synopsis:  BOOL  FileMigrateGP_205_210_Para(UNINIRAM_HGV_2_0_5 *pOldPara, UNINIRAM_HGV_2_1_0 *pNewPara)
*
*   Input   :   UNINIRAM_HGV_2_0_5  * pOldPara   - Rel 2.0.5 type of Param.
**
*   Output  :   UNINIRAM_HGV_2_1_0  * pNewPara   - Rel 2.1.0 type of Param.
**
*   Return  :   TRUE           - user process is executed
*               FALSE          - default process is expected
*
**  Description:  This function converts the parameter data in a buffer
*                 from Release 2.0.5 to Release 2.1.0, the current release.
*                 The conversion is done from the provided UNINIRAM buffer
*                 for Release 2.0.5 to a buffer for Release 2.1.0.
*
* ===========================================================================
*/
BOOL  FileMigrateGP_205_210_Para (UNINIRAM_HGV_2_0_5 *pOldPara, UNINIRAM_HGV_2_1_0 *pNewPara)
{
    USHORT     j,i;
	LONG	   fileOffset = 0;
	FILE       *pLogFile;
...
\
	...
	memset (pNewPara, 0, sizeof(UNINIRAM_HGV_2_1_0));
