...
{
    USHORT  i,j;
    WCHAR   aszTemp[PLU_MAX_DIGIT+1];

    //memset(aszTemp,(UCHAR)'0',sizeof(aszTemp)); /* all brank */
    wcscpy(aszTemp, WIDE("0000000000000"));
    ...
    while(j<PLU_MAX_DIGIT) {        /* PLU number exist */
        if(j & 0x01) {              /* even or odd ? */
            aszTemp[j] = (WCHAR)(((*(puchBCD+i)&0xf0) >> 4) + '0');
        }
        else {                      /* even */
            aszTemp[j] = (WCHAR)((*(puchBCD+i)&0x0f) + '0');
            i++;
        }
        j++;
    }
...
SHORT   RflNCFBCD2AsRight(UCHAR *pszAscii, UCHAR *puchBCD)
{
    ...
    //memset(aszTemp,(UCHAR)'0',sizeof(aszTemp)); /* all brank */
	wcscpy(aszTemp, WIDE("000000000000"));
