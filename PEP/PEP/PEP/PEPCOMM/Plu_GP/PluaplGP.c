...
USHORT  GetBatchNumber_GP(USHORT, WCHAR *);
BOOL    IsActiveRecord_GP(USHORT, WCHAR *);
BOOL    IsSatisfiedRecord_GP(WCHAR *, WCHAR *, PRECDIC);
USHORT  QueryCritUsers_GP(USHORT);
BOOL    IsLockedFile_GP(USHORT, USHORT);
BOOL    IsLockedRecord_GP(USHORT, PITEMNO, USHORT);
USHORT  ReturnUpdateRecord_GP(VOID *, PRECPLU, PRECPLU, PRECMIX, PRECPPI);
BOOL    IsValidCondition_GP(WCHAR *, USHORT, PRECDIC);
BOOL    IsSatisfiedData_GP(VOID *, VOID *, WCHAR, USHORT, UCHAR);
/* R2.0 */
VOID    SetIndexRecord_GP(RECIDX *pRecIdx, RECPLU *pRecPlu);
...
VOID    ExeInitialize_GP(BOOL fRefresh)
{
...
;
;   format : USHORT     ExeCreateFile(usFile, usNoOfRec);
;
;   input  : USHORT     usFile;         - file ID to create
;            USHORT     usNoOfRec;      - no. of records
;
...
USHORT  ExeCreateFile_GP(USHORT usFile, USHORT usNoOfRec)
{
    USHORT  usStat;
    FSPECS  specInfo;
    ...
    if (MnpIsMirrorFile_GP(usFile)) {              /* is mirror file given ?   */
    ...
    if (IsLockedFile_GP(usFile, FILE_READABLE + FILE_WRITABLE)) {
        return (SPLU_FILE_LOCKED);
    }

    /* --- is any locked rec. in the file ? --- */

    if (IsLockedRecord_GP(usFile, (PITEMNO)NULL, 0)) {
    ...
    if (ADJUSTFILE(usFile) == FILE_PLU) {
        ...
        if (! (usStat = MnpGetFileInfo_GP(FILE_PLU_INDEX, &specInfo))) {

            /* --- is file locked ? --- */

            if (IsLockedFile_GP(FILE_PLU_INDEX, FILE_READABLE + FILE_WRITABLE)) {
                return (SPLU_FILE_LOCKED);
            }

            /* --- is any locked rec. in the file ? --- */

            if (IsLockedRecord_GP(FILE_PLU_INDEX, (PITEMNO)NULL, 0)) {
                return (SPLU_REC_LOCKED);
            }

            /* --- delete index file --- */

            usStat = MnpDeleteFile_GP(FILE_PLU_INDEX);             /* just delete it       */
    ...
    switch (usNoOfRec) {                        /* how many records ?       */
    case 0:                                     /* create a 0 rec. file ?   */
        usStat = MnpDeleteFile_GP(usFile);             /* just delete it       */
        break;
    default:                                    /* others ...               */
        usStat = MnpCreateFile_GP(usFile, usNoOfRec);  /* create the file      */
        break;
    }
...
USHORT  ExeDeleteFile_GP(USHORT usFile)
{
    USHORT  usStat;
    FSPECS  specInfo;
...
USHORT  ExeClearFile_GP(USHORT usFile)
{
    USHORT  usStat;
    USHORT  usIndexFile;
    FSPECS  specInfo;
    ...
    if (ADJUSTFILE(usFile) == FILE_PLU) {
        ...
        usIndexFile = ExeSetIndexFileId_GP(usFile);

        if (! (usStat = MnpGetFileInfo_GP(usIndexFile, &specInfo))) {

            /* --- is file locked ? --- */

            if (IsLockedFile_GP(usIndexFile, FILE_READABLE + FILE_WRITABLE)) {
                return (SPLU_FILE_LOCKED);
            }

            /* --- is any locked rec. in the file ? --- */

            if (IsLockedRecord_GP(usIndexFile, (PITEMNO)NULL, 0)) {
                return (SPLU_REC_LOCKED);
            }

            /* --- clear index file --- */

            usStat = MnpClearFile_GP(usIndexFile);             /* just clear it            */
    ...
    usStat = MnpClearFile_GP(usFile);              /* just clear it            */
...
USHORT  ExeSenseFile_GP(USHORT usFile, PMFINFO pFileInfo)
{
    USHORT  usStat;
    FSPECS  specInfo;
    ...
    if (IsLockedFile_GP(usFile, FILE_READABLE)) {  /* can be read from a file ?*/
        return (SPLU_FILE_LOCKED);                  /* No ! being locked    */
    }

    /* --- get a file info --- */

    if (! (usStat = MnpGetFileInfo_GP(usFile, &specInfo))) {

        /* --- respond it --- */

        pFileInfo->usFileNo   = specInfo.usFileNo;      /* file number      */
        pFileInfo->ulFileSize = specInfo.ulFileSize;    /* size of file     */
        pFileInfo->usMaxRec   = specInfo.usMaxRec;      /* no. of records   */
        pFileInfo->usCurRec   = specInfo.usCurRec;      /* no. of cur. rec. */
    }
...
USHORT  ExeOpenMirror_GP(WCHAR FAR *pszFileName, WCHAR FAR *pszIndexName, USHORT *pusMirrorFile)
{
    USHORT  usStat;
    ...
    usStat = MnpOpenMirror_GP(pszFileName, pusMirrorFile);
    ...
    if (!usStat) {
        ...
        if (pszIndexName) {
            ...
            usStat = MnpOpenMirror_GP(pszIndexName, &usIndexMirror);
        ...
        if (!usStat) {
            ...
            if (i>=MAX_USERS) {
                ...
                MnpCloseMirror_GP(*pusMirrorFile);
                if (pszIndexName) {
                    MnpCloseMirror_GP(usIndexMirror);
                }
...
USHORT  ExeCloseMirror_GP(USHORT usMirrorFile)
{
    USHORT  usStat, i;

    /* --- just pass a control to manipulation --- */

    usStat = MnpCloseMirror_GP(usMirrorFile);
    ...
    for (i=0; i<MAX_USERS; i++) {
        ...
        if (aMirrorIdx[i].usPluFile == usMirrorFile) {
            ...
            MnpCloseMirror_GP(aMirrorIdx[i].usIndexFile);
...
USHORT  ExeEnterCritMode_GP(USHORT usFile,
                         USHORT usFunc,
                         USHORT *pusHandle,
                         VOID   *pvParam)
{
    ...
    *pusHandle = 0;                             /* assume no handle respond */
    ...
    if (usStat = MnpQInfoTable_GP(usFile, &pTable)) {  /* get information table*/
        return (usStat);
    }
    ...
    if ((usFunc == FUNC_REPORT_COND) || (usFunc == FUNC_REPORT_MAINTE) ||
        ...
        if (! IsValidCondition_GP(pucData, usBytes, pRecord)) {
            return (SPLU_INVALID_DATA);
        }
    }
    ...
    pUser->usIndexFile = ExeSetIndexFileId_GP(usFile);  /* access file ID for index    */
    ...
    if (IsLockedFile_GP(usFile, usAccess)) {       /* is the file accessable ? */
        return (SPLU_FILE_LOCKED);                  /* assume file locked   */
    }

    if (ADJUSTFILE(usFile) == FILE_PLU) {

        if (IsLockedFile_GP(ExeSetIndexFileId_GP(usFile), FILE_READABLE)) {       /* is the file accessable ? */
            return (SPLU_FILE_LOCKED);                  /* assume file locked   */
        }
    }
    ...
    if (usStat = MnpBeginLookUp_GP(usFile, &usManip, &usSeq, usFileVer)) {
        return (usStat);
    }
    ...
    switch (usFunc) {                           /* what kind of function ?  */
    case FUNC_REPORT_ADJECT:                    /* report adjective rec. ?  */
        MnpMoveFilePtr_GP(usManip, &(((PECMADJECT)pvParam)->itemPluNumber));
        break;
    case FUNC_BLOCK_ACCESS:                     /* block read / write ?     */
    ...
    case FUNC_REPORT_COND:                      /* report rec. w/ cond.     */
        if ((usSeq) && (pUser->usLength)) {
            if ((pUser->aucFuncData[0] == PLUID_DEPT_NUMBER) &&
                ...
                RecIdx.uchDeptNumber = pUser->aucFuncData[1];
                MnpMoveFilePtr_GP(usManip, &RecIdx);
            }
        }
        break;
...
USHORT  ExeExitCritMode_GP(USHORT usHandle, USHORT usReserve)
{
    USHORT  usStat;
    PUSRBLK pUser;
    ...
    usStat = MnpEndLookUp_GP(pUser->hManip);
...
USHORT  ExeKillCritMode_GP(USHORT usFile, USHORT usReserve)
{
    USHORT  i, usStat;
    PUSRBLK pUser;
...
USHORT  ExeAddRecord_GP(USHORT usFile, VOID *pvRec)
{
    USHORT  usStat;
    USHORT  usIndexFile;
    RECIDX  RecIdx;
    ...
    usStat = ExeAddRecordIn_GP(usFile, pvRec);

    /* --- add index record to index file, R2.0 ---  */

    if ((!usStat) && (ADJUSTFILE(usFile) == FILE_PLU)) {

        SetIndexRecord_GP(&RecIdx, (RECPLU *)pvRec);

        usStat = ExeAddRecordIn_GP(usIndexFile, &RecIdx);
...
USHORT  ExeAddRecordIn_GP(USHORT usFile, VOID *pvRec)
{
    USHORT  usStat;
    ...
    usStat = MnpAddRecord_GP(usFile, pvRec, NULL); /* just add it              */
...
USHORT
ExeReplaceRecord_GP(USHORT usFile, VOID *pvRec, VOID *pvBuffer, USHORT *pusRet)
{
    USHORT  usStat, usRet;
    USHORT  usIndexFile;
    RECIDX  RecIdx;
    ...
    usStat = ExeReplaceRecordIn_GP(usFile,
                                 pvRec,
                                 pvBuffer,
                                 &uchPreDept,
                                 pusRet);
    ...
    if ((!usStat) && (ADJUSTFILE(usFile) == FILE_PLU)) {
        ...
        if (uchPreDept) {   /* if old record exists */

            if (((RECPLU *)pvRec)->uchDeptNumber != uchPreDept) {

                /* delete old dept record */

                SetIndexRecord_GP(&RecIdx, (RECPLU *)pvRec);
                RecIdx.uchDeptNumber = uchPreDept;

                usStat = ExeDeleteRecordIn_GP(usIndexFile,
                                             &RecIdx,
                                             0,
                                             0,
                                             NULL,
...
USHORT
ExeReplaceRecordIn_GP(USHORT usFile,
                     VOID *pvRec,
                     VOID *pvBuffer,
                     WCHAR *puchDept,
                     USHORT *pusRet)
{
    ...
    *pusRet = 0;                                /* assume no data returned  */
    ...
    if (IsLockedRecord_GP(usFile, (PITEMNO)pucKey, usAdj)) {
        return (SPLU_REC_LOCKED);
    }
    ...
    if (ADJUSTFILE(usFile) == FILE_PLU_INDEX) {
        ...
        for (i=0; i<MAX_DEPT; i++) {
            ...
            RecIdx.uchDeptNumber = (UCHAR)i;

            usStat = MnpReadRecord_GP(usFile, HMNP_REC_MAIN, (WCHAR *)&RecIdx, 0, NULL);
        ...
        usStat = MnpReadRecord_GP(usFile, HMNP_REC_MAIN, pucKey, usAdj, aucRecord);
    ...
    case 0:                                     /* completed to read ?      */
        ...
        if (ADJUSTFILE(usFile) == FILE_PLU_INDEX) {
            ...
            usStat = MnpDeleteRecord_GP(usFile, (WCHAR *)&RecIdx, 0);  /* delete index file*/

            if (usStat == SPLU_COMPLETED) {

                usStat = MnpAddRecord_GP(usFile, pvRec, NULL); /* add on index file*/
            }
            if (pvBuffer) {
            ...
            *pusRet = pTable->usBytesRec;
            ...
            if (ADJUSTFILE(usFile) == FILE_PLU) {
                ...
                *puchDept = ((RECPLU *)aucRecord)->uchDeptNumber;
            }

            if (! (usStat = MnpModifyRecord_GP(HMNP_REC_MAIN, pvRec))) {
...
USHORT  ExeChangeRecord_GP(USHORT usFile,
                        VOID   *pvRec,
                        ULONG  flChanges,
                        USHORT fsProg,
                        VOID   *pvBuffer,
                        USHORT *pusRet)
{
    ...
    usStat = ExeChangeRecordIn_GP(usFile,
                             pvRec,
                             flChanges,
                             fsProg,
                             pvBuffer,
...
USHORT  ExeChangeRecordIn_GP(USHORT usFile,
                        VOID   *pvRec,
                        ULONG  flChanges,
                        USHORT fsProg,
                        VOID   *pvBuffer,
    ...
    *pusRet     = 0;                            /* assume no data returned  */
    ...
    if (ADJUSTFILE(usFile) == FILE_PLU_INDEX) {
        ...
        for (i=0; i<MAX_DEPT; i++) {
            ...
            usStat = MnpReadRecord_GP(usFile, HMNP_REC_MAIN, pvRec, 0, pvBuffer);
        ...
        if (usStat = MnpReadRecord_GP(usFile,          /* read the record          */
    ...
    if (fPluFile && fResetTotal) {              /* reset total fields ?     */
        ((PRECPLU)aucRecord)->lItemCounts = 0L;     /* reset item counter   */
        ((PRECPLU)aucRecord)->lSalesTotal = 0L;     /* reset sales total    */
        ((PRECPLU)aucRecord)->lDiscTotal  = 0L;     /* reset discount total */
    }
    ...
    if ((ADJUSTFILE(usFile) == FILE_PLU_INDEX)) {
        ...
        usStat = MnpDeleteRecord_GP(usFile, (WCHAR *)&RecIdx, usAdj);    /* delete index file*/
        ...
        usStat = MnpModifyRecord_GP(HMNP_REC_MAIN, aucRecord);
    }
...
USHORT  ExeDeleteRecord_GP(USHORT  usFile,
                        VOID    *pItemNumber,
                        USHORT  usAdj,
                        USHORT  fsProg,
                        VOID    *pvBuffer,
                        USHORT  *pusRet)
{
    ...
    usStat = ExeDeleteRecordIn_GP(usFile,
                             pItemNumber,
                             usAdj,
                             fsProg,
                             pvBuffer,
    ...
    if ((!usStat) && (ADJUSTFILE(usFile) == FILE_PLU)) {
        ...
        RecIdx.uchDeptNumber = uchPreDept;
        memcpy(&RecIdx.aucPluNumber, pItemNumber, 7);
...
USHORT  ExeDeleteRecordIn_GP(USHORT  usFile,
                        VOID    *pItemNumber,
                        USHORT  usAdj,
                        USHORT  fsProg,
                        VOID    *pvBuffer,
                        ...
                        USHORT  *pusRet)
{
    ...
    *pusRet = 0;                                /* assume no data returned  */
    ...
    if (ADJUSTFILE(usFile) == FILE_PLU_INDEX) {
        ...
        if (IsLockedRecord_GP(usFile, (PITEMNO)pucKey, usIndAdj)) {
            return (SPLU_REC_LOCKED);
        }
    } else {
        if (IsLockedRecord_GP(usFile, (PITEMNO)pItemNumber, usAdj)) {
            return (SPLU_REC_LOCKED);
        }
    }
    ...
    if ((fsProg & PROG_PROTECT_DELETE) || pvBuffer) {
        ...
        if (pvBuffer) {                         /* is buffer provided ?     */
            memcpy(pvBuffer, aucRecord, pTable->usBytesRec);/* respond rec. */
            *pusRet = pTable->usBytesRec;                   /* its length   */
        }
...
/*          if (pPlu->lItemCounts || pPlu->lSalesTotal || pPlu->lSalesTotal) {  */
    ...
    usStat = MnpDeleteRecord_GP(usFile, (WCHAR *)pItemNumber, usAdj);
...
USHORT  ExeDeleteReserved_GP(USHORT usHandle)
{
    USHORT      usStat;
    PUSRBLK     pUser;
    RECIDX  RecIdx;
    ...
    if ((ADJUSTFILE(pUser->usFile) == FILE_PLU_INDEX)) {
        ...
        memcpy(&RecIdx.aucPluNumber, &(pUser->itemNumber), 7);
        RecIdx.uchAdjective = (UCHAR)pUser->usAdjective;
        RecIdx.uchDeptNumber = pUser->uchDeptNumber;

        usStat = MnpDeleteRecord_GP(pUser->usFile, (WCHAR *)&RecIdx, 0);

    } else {

        usStat = MnpDeleteRecord_GP(pUser->usFile,         /* delete the record    */
    ...
    if (usStat == SPLU_COMPLETED) {
        ...
        if (! (usStat = MnpGetFileInfo_GP(pUser->usIndexFile, &specInfo))) {

            memcpy(&RecIdx.aucPluNumber, &(pUser->itemNumber), 7);
            RecIdx.uchAdjective = (UCHAR)pUser->usAdjective;
            RecIdx.uchDeptNumber = pUser->uchDeptNumber;

            usStat = MnpDeleteRecord_GP(pUser->usIndexFile, (WCHAR *)&RecIdx, 0);   /* delete index file*/
...
USHORT  ExeResetReserved_GP(USHORT usHandle)
{
    BOOL        fPlu;
    ...
    if (usStat = MnpPeekRecord_GP(pUser->hManip, aucRecord)) {
        return (usStat);
    }

    /* --- reset only total fields --- */

    ((PRECPLU)aucRecord)->lItemCounts = 0L;     /* reset item counts        */
    ((PRECPLU)aucRecord)->lSalesTotal = 0L;     /* reset sales total        */
    ((PRECPLU)aucRecord)->lDiscTotal  = 0L;     /* reset discount total     */

    /* --- override its record --- */

    usStat = MnpModifyRecord_GP(pUser->hManip, aucRecord);
...
USHORT  ExeRewriteReserved_GP(USHORT usHandle, VOID *pvRecord)
{
    USHORT      usStat;
    PKEYINF     pTable;
    PUSRBLK     pUser;
    ...
    case FILE_PPI:                              /* PPI file ?               */
    case FILE_PLU_INDEX:                        /* PLU index file ? */
        if (usStat = MnpQInfoTable_GP(pUser->usFile, &pTable)) {
            return (usStat);
        }
        break;                                      /* these are valid !    */
    default:                                    /* others ...               */
    ...
    if (ADJUSTFILE(pUser->usFile) == FILE_PLU_INDEX) {
        ...
        usStat = MnpModifyRecord_GP(pUser->hManip, pvRecord);
    }
    ...
    if (usStat == SPLU_COMPLETED) {
        ...
        if ((ADJUSTFILE(pUser->usFile) == FILE_PLU) ||
            ...
            if (! (usStat = MnpGetFileInfo_GP(usFile, &specInfo))) {
                ...
                if (pUser->uchDeptNumber != ((RECPLU *)pvRecord)->uchDeptNumber) {
                    ...
                    if ((usStat == SPLU_COMPLETED) || (usStat == SPLU_REC_NOT_FOUND)) {
                        ...
                        SetIndexRecord_GP(&RecIdx, (RECPLU *)pvRecord);

                        usStat = MnpAddRecord_GP(usFile, &RecIdx, NULL);   /* add on index file*/
                    }
                }
            }
        }
...
USHORT  ExeCancelReserved_GP(USHORT usHandle)
{
    PUSRBLK     pUser;
...
USHORT  ExeAppendMainte_GP(USHORT usFile, VOID *pvRec)
{
...
USHORT  ExePurgeMainte_GP(USHORT usHandle)
{
    BOOL        fApplied;
    WCHAR       aucRecord[ENOUGHREC];
    ...
    for ( ; usLast; usLast--) {
        ...
        if (usStat = MnpGetLookUp_GP(pUser->hManip, aucRecord)) {
            break;
        }
        ...
        if (((pUser->fsOption & PURGE_UNAPPLY) && (! fApplied))
            ...
            if (usStat = MnpRemoveRecord_GP(pUser->hManip)) {
                break;
            }
        }
    }
...
USHORT  ExeApplyMainte_GP(USHORT usHandle,
                       VOID   *pvBuffer,
                       USHORT *pusRet,
                       VOID   *pvMainte,
                       USHORT *pusLen,
                       USHORT *pusApplied)
{
    ...
    for ( ; usLast; usLast--) {
        ...
        case MNT_TYPE_ADD:                      /* add a record ?           */
            usStat  = ExeAddRecord_GP(usFile, pMainte);
            *pusRet = 0;
            break;
        case MNT_TYPE_REPLACE:                  /* replace a record ?       */
            usStat = ExeReplaceRecord_GP(usFile, pMainte, pvBuffer, pusRet);
            break;
        case MNT_TYPE_CHANGE:                   /* change a record ?        */
            usStat = ExeChangeRecord_GP(usFile,
                                     pMainte,
            ...
            break;
        case MNT_TYPE_DELETE:                   /* delete a record ?        */
            usStat = ExeDeleteRecord_GP(usFile,
                                     (PITEMNO)pMainte,
...
;
;   format : USHORT     ExeRemoveMainte(usHandle, usKey);
;
;   input  : USHORT     usHandle;       - handle
;            USHORT     usKey;          - key of record to remove
;
;   output : USHORT     usStat;         - status
;
;========================================================================
...
USHORT  ExeRemoveMainte_GP(USHORT usHandle, USHORT usKey)
{
    USHORT      usStat, usCell;
    PUSRBLK     pUser;
    ...
    usCell = usKey - ADJUST_BOUND_KEY;          /* compute cell no.         */

    /* --- adjust file ptr. --- */

    if (usStat = MnpSetFilePtr_GP(pUser->hManip, usCell)) {
        return (usStat);
    }

    /* --- remove the record --- */

    usStat = MnpRemoveRecord_GP(pUser->hManip);
...
;
;   format : USHORT     ExeModifyMainte(usHandle, usKey, pvRec);
;
;   input  : USHORT     usHandle;       - handle
;            USHORT     usKey;          - key of record to modify
;            VOID       *pvRec;         - ptr. to record data
;
;   output : USHORT     usStat;         - status
;
...
USHORT  ExeModifyMainte_GP(USHORT usHandle, USHORT usKey, VOID *pvRec)
{
    USHORT      usStat, usCell, usType, *pusStatus;
    PUSRBLK     pUser;
    ...
    usStat = MnpModifyRecord_GP(pUser->hManip, pvRec);
...
;
;   format : USHORT     ExeReadMainte(usHandle, usKey, pvBuffer, pusRet);
;
;   input  : USHORT     usHandle;       - handle
;            USHORT     usKey;          - key of record to modify
...
USHORT
ExeReadMainte_GP(USHORT usHandle, USHORT usKey, VOID *pvBuffer, USHORT *pusRet)
{
    USHORT      usStat, usCell;
    PKEYINF     pTable;
    PUSRBLK     pUser;
    ...
    *pusRet = 0;                                /* assume no data returned  */
    ...
    if (! (usStat = MnpPeekRecord_GP(pUser->hManip, pvBuffer))) {
        *pusRet = pTable->usBytesRec;
    }
...
;
;   format : USHORT     ExeAddMainte(usHandle, pvRec, pusKey);
;
;   input  : USHORT     usHandle;       - handle
;            VOID       *pvRec;         - ptr. to record data
;            USHORT     *pusKey;        - buffer ptr. to respond key
;
;   output : USHORT     usStat;         - status
;
;========================================================================
...
USHORT  ExeAddMainte_GP(USHORT usHandle, VOID *pvRec, USHORT *pusKey)
{
    USHORT      usStat, usCell, usType, *pusStatus;
    PUSRBLK     pUser;
    ...
    *pusStatus = SPLU_MNT_UNAPPLY;              /* not yet apply it         */
    ...
    usStat  = MnpAddRecord_GP(pUser->usFile, pvRec, &usCell);
    *pusKey = usCell + ADJUST_BOUND_KEY;        /* compute cell no.         */
...
USHORT  ExeReadRecord_GP(USHORT  usFile,
                      PITEMNO pItemNumber,
                      USHORT  usAdj,
                      VOID    *pvBuffer,
                      USHORT  *pusRet)
...
USHORT  ExeReserveRecord_GP(USHORT  usHandle,
                         PITEMNO pItemNumber,
                         USHORT  usAdj,
                         VOID    *pvBuffer,
                         USHORT  *pusRet)
{
    ...
    *pusRet = 0;                                /* assume no data returned  */
    ...
    if (IsLockedRecord_GP(pUser->usFile, pItemNumber, usAdj)) {
        return (SPLU_REC_LOCKED);
    }
    ...
    if (usStat = MnpReadRecord_GP(pUser->usFile,   /* read the record          */
    ...
    *pusRet = pTable->usBytesRec;               /* record length in byte    */
    ...
    pUser->uchDeptNumber = ((RECPLU *)pvBuffer)->uchDeptNumber; /* remember dept. no for index R2.0 */
...
;
;   format : USHORT     ExeReportRecord(usHandle, pvBuffer, pusKey, pusRet);
;
;   input  : USHORT     usHandle;       - handle value
;            VOID       *pvBuffer;      - buffer ptr. to respond rec.
;            USHORT     *pusKey;        - key no. reported
;            USHORT     *pusRet;        - buffer ptr. to respond rec. length
;
;   output : USHORT     usStat;         - status
;
...
USHORT  ExeReportRecord_GP(USHORT usHandle,
                        VOID   *pvBuffer,
                        USHORT *pusKey,
                        USHORT *pusRet)
{
    ...
    WCHAR       aucRecord[ENOUGHREC];
    USHORT      usStat, usLast, usBatch, usCell;
    PUSRBLK     pUser;
    PKEYINF     pTable;
    PRECDIC     pRecord;
    FSPECS      specInfo;
    ...
    *pusRet = 0;                                /* assume no data returned  */
    ...
    if (fReset) {
        ...
        if (usStat = MnpModifyRecord_GP(pUser->hManip, aucRecord)) {
            return (usStat);
        }
    }
    ...
    for ( ; usLast; usLast--) {
        ...
        case FUNC_REPORT_COND:                  /* report rec. w/ cond. ?   */
        case FUNC_REPORT_COND2:                 /* report rec. w/ cond. ?   */
            fActive  = IsActiveRecord_GP(pUser->usFile, aucRecord);
            ...
            break;
        case FUNC_REPORT_MAINTE:                /* report mainte. rec. ?    */
            usBatch  = GetBatchNumber_GP(pUser->usFile, aucRecord);
        ...
        if (fSatisfy && pUser->usLength) {
            fSatisfy = IsSatisfiedRecord_GP(aucRecord,
                                         pUser->aucFuncData,
                                         pRecord);
        }
        ...
        if (pUser->uchFunc == FUNC_REPORT_ADJECT) { /* report adj. rec. ?   */
            ...
            if (! IsSatisfiedRecord_GP(aucRecord, pUser->aucFuncData, pRecord)) {
            ...
            if (! (usStat = MnpGetFileInfo_GP(pUser->usIndexFile, &specInfo))) {
                if (pUser->usLength) {
                    if ((pUser->aucFuncData[0] == PLUID_DEPT_NUMBER) &&
                        ...
                        if (!(IsSatisfiedRecord_GP(aucRecord,
                                                 pUser->aucFuncData,
                                                 pRecord))) {
        ...
        if (pUser->uchFunc == FUNC_REPORT_RANGE) {
            if (! (usStat = MnpGetFileInfo_GP(pUser->usIndexFile, &specInfo))) {
                if (pUser->usLength) {
                    ...
                    for (++pUser->uchDeptNumber; pUser->uchDeptNumber <MAX_DEPT; pUser->uchDeptNumber++) {

                        RecIdx.uchDeptNumber = pUser->uchDeptNumber;
                        memcpy(&RecIdx.aucPluNumber, &pUser->aucFuncData[1], pTable->usKeyLen);
                        RecIdx.uchAdjective = 0;

                        if (!(usStat = MnpMoveFilePtr_GP(pUser->hManip, &RecIdx))) {

                            break;
                        }
                    }
                }
                if (pUser->uchDeptNumber > MAX_DEPT) {
                    usStat = SPLU_END_OF_FILE;              /* assume EOF       */
                    break;                                  /* break my job     */
                }
            }
    ...
    if ((pUser->uchFunc == FUNC_REPORT_R10) &&
        ...
        *pusRet = sizeof(RECPLU10);                 /* record length in byte    */
        memcpy(pvBuffer, aucRecord, sizeof(RECPLU10));
    ...
    if (pusKey) {
        usStat  = MnpGetFilePtr_GP(pUser->hManip, &usCell);
        *pusKey = usCell + ADJUST_BOUND_KEY;
    }
    ...
    if (fLock) {
        ...
        if ((pUser->usFile == FILE_PLU) ||
            ...
            pUser->uchDeptNumber = ((RECPLU *)aucRecord)->uchDeptNumber; /* remember dept. no for index R2.0 */
        ...
        fAttached = IsLockedRecord_GP(pUser->usFile,   /* someone locked this ?*/
        ...
        if (! fAttached) {                      /* if lockable rec. ...     */
            ...
            MnpKeepFilePtr_GP(pUser->hManip);          /* try it later         */
        }
...
USHORT
ExeReadBlock_GP(USHORT usHandle, WCHAR *pucBuffer, USHORT usSize, USHORT *pusRead)
{
    USHORT      usStat;
    PUSRBLK     pUser;
    ...
    *pusRead = 0;                               /* assume no data returned  */
    ...
    usStat = MnpReadFile_GP(pUser->hManip, pucBuffer, usSize, pusRead);
...
USHORT
ExeWriteBlock_GP(USHORT usHf, WCHAR *pucData, USHORT usBytes, USHORT *pusWritten)
{
    USHORT      usStat;
    PUSRBLK     pUser;
    ...
    *pusWritten = 0;                            /* assume no data written   */
    ...
    usStat = MnpWriteFile_GP(pUser->hManip, pucData, usBytes, pusWritten);
...
USHORT
ExeSeekBlock_GP(USHORT usHandle, LONG lMoves, USHORT usOrigin, ULONG *pulNewPtr)
{
    USHORT      usStat;
    PUSRBLK     pUser;
    ...
    usStat = MnpSeekFile_GP(pUser->hManip, lMoves, usOrigin, pulNewPtr);
...
USHORT  ExeUpdateRecord_GP(USHORT  usFile,
                        PITEMNO pNumber,
                        USHORT  usAdj,
                        PUPDINF pMainInfo,
                        PUPDINF pLinkInfo,
                        ...
                        USHORT  *pfsStatus)
{
    ...
    WCHAR       aucKey[LONGOF(aucPluNumber, RECPLU)];
    WCHAR       *pucKey, *pfcFlags;
    WCHAR       *pucNext, aucEnough[2*sizeof(RECPLU) + sizeof(RECMIX) + sizeof(RECPPI)];
    ...
    *pfsStatus = 0;                             /* initialize resp. status  */
    ...
    fLocked   = IsLockedRecord_GP(FILE_PLU, pNumber, usAdj);
    ...
    if (IsLockedFile_GP(FILE_PLU, fsAction)) {     /* is PLU file seized ?     */
        return (SPLU_FILE_LOCKED);                  /* file lock violation  */
    }

    /* --- read the main record --- */

    if (usStat = MnpReadRecord_GP(FILE_PLU,        /* read the record          */
                               HMNP_REC_MAIN,       /* remember its position*/
                               (WCHAR *)pNumber,    /* ptr. to key          */
                               usAdj,               /* adjective number     */
    ...
    if (memcmp(pPlu->aucLinkNumber, aucKey, LONGOF(aucLinkNumber, RECPLU))) {

        /* --- make complete key data --- */

        pucKey = aucKey
                + LONGOF(aucPluNumber, RECPLU)
                - LONGOF(aucLinkNumber, RECPLU);
        memcpy(pucKey, pPlu->aucLinkNumber, LONGOF(aucLinkNumber, RECPLU));

        /* --- is it a locked rec. ? --- */

        fLocked = IsLockedRecord_GP(FILE_PLU, (PITEMNO)aucKey, 0);
        ...
        usStat = MnpReadRecord_GP(FILE_PLU, HMNP_REC_SUB, aucKey, 0, pucNext);
    ...
    if (pPlu->uchMixMatchTare && (! DEFFLG(pfcFlags, PLUST_SCALE_ITEM))) {
        ...
        if (IsLockedFile_GP(FILE_MIX_MATCH, FILE_READABLE)) {
            return (SPLU_FILE_LOCKED);
        }
        ...
        usStat = MnpReadRecord_GP(FILE_MIX_MATCH, 0, pucKey, 0, pucNext);
    ...
    if (pPlu->uchLinkPPI) {                     /* linking with PPI Rec. ?  */
        ...
        if (IsLockedFile_GP(FILE_PPI, FILE_READABLE)) {
            return (SPLU_FILE_LOCKED);
        }

        /* --- read the main record --- */

        usStat = MnpReadRecord_GP(FILE_PPI, 0, &(pPlu->uchLinkPPI), 0, pucNext);
    ...
    if ((fsMainInfo & CNTRL_EXEC_READ) && pvBuffer) {
        ReturnUpdateRecord_GP(pvBuffer, pPlu, pLink, pMix, pPpi);
        *pfsStatus |= pLink ? RETURN_LINK      : 0;
        *pfsStatus |= pMix  ? RETURN_MIX_MATCH : 0;
        *pfsStatus |= pPpi  ? RETURN_PPI       : 0;
    }
    ...
    *pfsStatus |= fsNotWhy;                     /* respond record status    */
    ...
    pPlu->lItemCounts += lQty;
    pPlu->lSalesTotal += lAmt;
    pPlu->lDiscTotal  += lDisc;
    pPlu->lSalesTotal += (fsMainInfo & CNTRL_WITH_DISCOUNT) ? lAmt : 0L;

    /* --- update a main PLU record --- */

    usStat = MnpModifyRecord_GP(HMNP_REC_MAIN, pPlu);
    ...
    pLink->lItemCounts += lQty;
    pLink->lSalesTotal += lAmt;
    pLink->lDiscTotal  += lDisc;
    pLink->lSalesTotal += (fsLinkInfo & CNTRL_WITH_DISCOUNT) ? lAmt : 0L;

    /* --- update it --- */

    usStat = MnpModifyRecord_GP(HMNP_REC_SUB, pLink);
...
USHORT  ExeInformStatus_GP(USHORT  usHandle,
...
;
;   format : USHORT     QueryCritUsers_GP(usFunc);
;
...
USHORT  QueryCritUsers_GP(USHORT usFunc)
{
    USHORT      i, usUsers;
    PUSRBLK     pUser;
...
;
;   format : BOOL       IsLockedRecord_GP(usFile, pItem, usAdj);
;
;   input  : USHORT     usFile;         - file ID
...
BOOL    IsLockedRecord_GP(USHORT usFile, PITEMNO pItem, USHORT usAdj)
{
    USHORT      i;
    PUSRBLK     pUser;
    PKEYINF     pTable;
    ...
    if (MnpQInfoTable_GP(usFile, &pTable)) {       /* get info. table ptr.     */
        return (FALSE);
    }
...
;
;   format : BOOL       IsLockedFile_GP(usFile, fsAction);
;
;   input  : USHORT     usFile;         - file ID
...
BOOL    IsLockedFile_GP(USHORT usFile, USHORT fsAction)
{
    BOOL        fLocked;
    UCHAR       fcProhibit;
    USHORT      i;
...
;
;   format : USHORT     ReturnUpdateRecord_GP(pvBuffer, pPlu, pLink, pMix, pPpi);
;
...
;            PRECPPI    pPpi;           - ptr. to PPI record
;
...
USHORT  ReturnUpdateRecord_GP(VOID    *pvBuffer,
                           PRECPLU pPlu,
                           PRECPLU pLink,
                           PRECMIX pMix,
                           PRECPPI pPpi)
{
    ...
    memcpy(pucTarget, (UCHAR *)(&(pPlu->uchBonusStatus)), sizeof(UCHAR));
    pucTarget += sizeof(UCHAR);
    memcpy(pucTarget, (UCHAR *)(&(pPlu->uchMixMatchTare)), PLU_AFTER_STATUS_LEN);
    pucTarget += PLU_AFTER_STATUS_LEN;
    memcpy(pucTarget, (UCHAR *)(&(pPlu->usFqsValue)), sizeof(USHORT));
...
;
;   format : BOOL       IsValidCondition_GP(pucCond, usBytes, pDictionary);
;
...
BOOL    IsValidCondition_GP(WCHAR *pucCond, USHORT usBytes, PRECDIC pDictionary)
{
    BOOL        fEndOfData;
...
;
;   format : USHORT     GetBatchNumber_GP(usFile, pucRecord);
;
;   input  : USHORT     usFile;         - file ID
...
USHORT  GetBatchNumber_GP(USHORT usFile, WCHAR *pucRecord)
{
    /* --- is PLU file ? --- */

    if (usFile != FILE_MAINT_PLU) {             /* if out of PLU file ...   */
        return (USHORT)0;                       /* no batch no.         */
    }
...
;
;   format : BOOL       IsActiveRecord_GP(usFile, pucRecord);
;
;   input  : USHORT     usFile;         - file ID
;            UCHAR      *pucRecord;     - ptr. to record data
;
...
BOOL    IsActiveRecord_GP(USHORT usFile, WCHAR *pucRecord)
{
    BOOL        fActive;
    PRECPLU     pPlu;
    ...
    pPlu = (PRECPLU)pucRecord;                  /* top of data ptr.         */
...
;
;   format : BOOL       IsSatisfiedRecord_GP(pucRec, pucCond, pDictionary);
;
...
BOOL
IsSatisfiedRecord_GP(WCHAR *pucRecord, WCHAR *pucCond, PRECDIC pDictionary)
{
    ...
    while (! fBreak) {                          /* check whole expression   */
        ...
        fSatisfied = IsSatisfiedData_GP(pucData,   /* compare with value data  */
...
;
;   format : BOOL       IsSatisfiedData_GP(pucRec, pucCond, pDictionary);
;
;   input  : UCHAR      *pucRecord;     - ptr. to record data
;            UCHAR      *pucCond;       - ptr. to conditional expression
;            PRECDIC    pDictionary;    - ptr. to record dictionary
...
BOOL    IsSatisfiedData_GP(VOID   *pvData,
                        VOID   *pvValue,
                        WCHAR  uchType,
                        USHORT usLength,
                        UCHAR  uchComp)
...
;
;   format : VOID       SetIndexRecord_GP(pRecIdx, pRecPlu);
;
...
VOID SetIndexRecord_GP(RECIDX *pRecIdx, RECPLU *pRecPlu)
{
    if (pRecPlu) {
        ...
        pRecIdx->uchDeptNumber = pRecPlu->uchDeptNumber;
        memcpy(pRecIdx->aucPluNumber, pRecPlu->aucPluNumber, 7);
