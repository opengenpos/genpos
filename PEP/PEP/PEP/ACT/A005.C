...
BOOL    WINAPI  A005DlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SETFONT:
		if (hResourceFont) {
			for(int j = IDD_A005_OFFDEF; j <= IDD_A005_DESC10; j++)
			{
...
static struct {
    ...
    {IDS_A005_OFFDEF, IDD_A005_OFFDEF},
    {IDS_A005_DEF01, IDD_A005_DEF01},
    {IDS_A005_DEF02, IDD_A005_DEF02},
    {IDS_A005_DEF03, IDD_A005_DEF03},
    {IDS_A005_DEF04, IDD_A005_DEF04},
    {IDS_A005_DEF05, IDD_A005_DEF05},
    {IDS_A005_DEF06, IDD_A005_DEF06},
    {IDS_A005_DEF07, IDD_A005_DEF07},
    {IDS_A005_DEF08, IDD_A005_DEF08},
    {IDS_A005_DEF09, IDD_A005_DEF09},

    {IDS_A005_OFFNOT, IDD_A005_OFFNOT},
    {IDS_A005_NOT01, IDD_A005_NOT01},
    {IDS_A005_NOT02, IDD_A005_NOT02},
    {IDS_A005_NOT03, IDD_A005_NOT03},
    {IDS_A005_NOT04, IDD_A005_NOT04},
    {IDS_A005_NOT05, IDD_A005_NOT05},
    {IDS_A005_NOT06, IDD_A005_NOT06},
    {IDS_A005_NOT07, IDD_A005_NOT07},
    {IDS_A005_NOT08, IDD_A005_NOT08},
    {IDS_A005_NOT09, IDD_A005_NOT09},

    /*** NCR2172 ***/
    {IDS_A005_ADJGRP, IDD_A005_ADJGRP},
    {IDS_A005_DESC01, IDD_A005_DESC01},
    {IDS_A005_DESC02, IDD_A005_DESC02},
    {IDS_A005_DESC03, IDD_A005_DESC03},
    {IDS_A005_DESC04, IDD_A005_DESC04},
    {IDS_A005_DESC05, IDD_A005_DESC05},
    {IDS_A005_DESC06, IDD_A005_DESC06},
    {IDS_A005_DESC07, IDD_A005_DESC07},
    {IDS_A005_DESC08, IDD_A005_DESC08},
    {IDS_A005_DESC09, IDD_A005_DESC09},
    {IDS_A005_DESC10, IDD_A005_DESC10}
};

VOID    A005InitDlg(HWND hDlg)
{
    BYTE    abData[A005_ADDR_MAX];
    WORD    wDefID, wNotID = IDD_A005_NOT01;
    USHORT  usRet;
    WCHAR    szDesc[PEP_LOADSTRING_LEN + 1] = { 0 };
    UINT   unValue;
    ...
    for (int i = 0; i < sizeof(StrCntrlTable) / sizeof(StrCntrlTable[0]); i++) {
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, StrCntrlTable[i].strid, StrCntrlTable[i].cntrlid);
    }

    /* ----- Read Initial Data from Parameter File ----- */
    ParaAllRead(CLASS_PARACTLTBLMENU, (UCHAR *)abData, sizeof(abData), 0, &usRet);
...
*/
    ...
    for (WORD wI = A005_PAGE_MIN; wI <= A005_PAGE_MAX; wI++, wNotID++) {
        CheckDlgButton(hDlg, wNotID, (UINT)abData[wI]);
    }

/*** NCR2172 ***/
    /* ----- Set Initial Data (Default Page No.) ----- */
    for (WORD wI = 0; wI < A05_OFFSET - 1; wI++) {
        /* ----- Limit Length of Input Data to EditText ----- */
        SendDlgItemMessage(hDlg, IDD_A005_ADJED01 + wI, EM_LIMITTEXT, A05_MAXLEN, 0L);

        /* ----- Set Adjective Data to Specified EditText ----- */
        unValue = (UINT)abData[A05_OFFSET + wI];
        SetDlgItemInt(hDlg, IDD_A005_ADJED01 + wI, (WORD)unValue, FALSE);
    }
}

/*
...
{
    BYTE    abData[A005_ADDR_MAX] = { 0 };
    WORD    wNotID = IDD_A005_NOT01, wDefID = IDD_A005_DEF01;
    USHORT  usRet;

    for (WORD wI = A005_PAGE_MIN; wI <= A005_PAGE_MAX; wI++, wNotID++, wDefID++) {
...
/*** NCR2172 Start ***/
    ...
    ParaAllWrite(CLASS_PARACTLTBLMENU, (UCHAR *)abData, sizeof(abData), 0, &usRet);
...
{
    BOOL   fCheck = FALSE;
    UINT   nPos = GetDlgItemInt(hDlg, wItem, NULL, TRUE);
...
{
    WORD    wMin = A05_DATA_MIN, wMax = A05_DATA_MAX;
    WCHAR   szWork[PEP_OVER_LEN] = { 0 },
        szMessage[PEP_LOADSTRING_LEN] = { 0 },
        szCaption[PEP_CAPTION_LEN] = { 0 };
