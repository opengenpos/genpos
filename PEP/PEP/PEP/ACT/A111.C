...
static VOID    A111ModifyError(HWND hDlg)
{
    if (IsDlgButtonChecked(hDlg, IDD_A111EB1)) {
        for (USHORT chWork = 0; chWork < A111_ADDR_MAX; chWork++) {
...
BOOL    WINAPI  A111DlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        ParaAllRead(CLASS_PARATARE, (UCHAR *)anTare, sizeof(anTare), 0, &usRetLen);
        ...
        if (A111CheckData(hDlg, A111_BUFFER)) {
            ...
            nRet = DialogBoxPopup(hResourceDll, MAKEINTRESOURCEW(IDD_A111_ERR), hDlg, (DLGPROC)A111ErrorDlgProc);
        }
	...
	case WM_SETFONT:
		if (hResourceFont) {
			for(int j = IDD_A111_EDIT01; j <= IDD_A111_TARE30; j++)
			{
    ...
    case WM_COMMAND:
        ...
        case IDD_A111EXTEND:
            ...
            wDlgWidth = (WORD)((LOWORD(wParam) == IDD_A111EXTEND) ? wWidth : (wWidth * A111_MULTIPLY / A111_DIVIDE));
            ...
            /* ----- Get ID to Set Focus ----- */
            wID = (WORD)((LOWORD(wParam) == IDD_A111EXTEND) ?  IDD_A111CLOSE : IDD_A111EXTEND);
        ...
        case IDD_A111_EDIT10:
            if (HIWORD(wParam) == EN_CHANGE) {
                ...
                if ((mdcData.uchMDCData & A111_MDC_D) && (! (mdcData.uchMDCData & A111_MDC_B))) {
        ...
        case IDD_A111RB4:
            if (HIWORD(wParam) == BN_CLICKED) {
                ...
                wLimit = (WORD)((mdcData.uchMDCData & A111_MDC_D) ? A111_DON_LEN : A111_DOFF_LEN);
        ...
        case IDOK:
            ...
            if (A111CheckData(hDlg, A111_EDITTEXT)) {
                ...
                /* ----- Write Tare Data ----- */
                ParaAllWrite(CLASS_PARATARE, (UCHAR *)anTare, sizeof(anTare), 0, &usRetLen);
                ParaMDCWrite(&mdcData);
...
BOOL WINAPI A111ErrorDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SETFONT:
		if (hResourceFont) {
			for(int j = IDD_A111_CAPTION1; j <= IDD_A111_GROUPBOX; j++)
			{
				SendDlgItemMessage(hDlg, j, WM_SETFONT, (WPARAM)hResourceFont, 0);
			}
...
VOID    A111InitDlg(HWND hDlg) 
{
   // LPUINT  lpunTare = (LPUINT)anTare; //SR#125
    ...
    wID = (WORD)((mdcData.uchMDCData & A111_MDC_B) ? IDD_A111RB2 : IDD_A111RB1);
    CheckRadioButton(hDlg, IDD_A111RB1, IDD_A111RB2, wID);

    /* ----- Check RadioButton (Bit-D : IDD_A111RB3 or IDD_A111RB4) -----*/
    wID = (WORD)((mdcData.uchMDCData & A111_MDC_D) ? IDD_A111RB4 : IDD_A111RB3);
...
WCHAR    A111CheckData(HWND hDlg, BOOL fType)
{
    ...
    for (nCount = 0, chCount = 0; nCount < A111_ADDR_MAX; nCount++) {
        if (fType == A111_EDITTEXT) {   /* Check EditText Data  */
            anTare[nCount] = (short)GetDlgItemInt(hDlg, IDD_A111_EDIT01 + nCount, NULL, FALSE);
        }
...
VOID    A111SetError(HWND hDlg, UINT unStatus, WORD wID)
{
    ...
    for (USHORT chCount = 0; chCount < A111_ADDR_MAX; chCount++) {
    ...
    /* ----- Popup MessageBox ----- */
    MessageBeep(MB_ICONEXCLAMATION);
    nRet = MessageBoxPopUp(hDlg, szWork, szCaption, afuStyle[unStatus]);
    ...
    if (unStatus == A111_OK) {      /* MessageBox Style is OK   */
        ...
        for (USHORT chCount = 0; chCount < A111_ADDR_MAX; chCount++) {
            ...
            SetDlgItemInt(hDlg, IDD_A111_EDIT01 + achErrData[chCount], anTare[achErrData[chCount]], FALSE);
        }
    }
}

...
VOID    A111CheckEdit(HWND hDlg, WORD wID)
{
    ...
    if (nTextLength == A111_DON_LEN) {
        ...
        if (unData % A111_LSTCHKER) {       /* If Least Digit is not 0 or 5 */
            WCHAR    szCaption[PEP_CAPTION_LEN] = { 0 }, szMessage[A111_BUFF_LEN] = { 0 };

            LoadString(hResourceDll, IDS_PEP_CAPTION_A111, szCaption, PEP_STRING_LEN_MAC(szCaption));
            LoadString(hResourceDll, IDS_A111_0OR5,        szMessage, PEP_STRING_LEN_MAC(szMessage));

            MessageBeep(MB_ICONEXCLAMATION);
            MessageBoxPopUp(hDlg, szMessage, szCaption, afuStyle[A111_OK]);
...
VOID    A111SetCondition(HWND hDlg)
{
    ...
    DlgItemLoadStringRedrawText(hDlg, hResourceDll, nID, IDD_A111CONDITION);
}

/*
* ===========================================================================
...
{
	hWndModeLess = DialogCreation(hResourceDll/*hActInst*/, MAKEINTRESOURCEW(IDD_A111), hParentWnd, A111DlgProc);  //RPH 4-23-03 Multilingual
