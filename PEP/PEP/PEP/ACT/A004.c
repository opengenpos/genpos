...
BOOL    WINAPI  A004DlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case    WM_INITDIALOG:
        ...
        SendDlgItemMessage(hDlg, IDD_A04_PLUKEY, EM_LIMITTEXT, (WPARAM)A04_KEYNO_LEN, 0L);

        SendDlgItemMessage(hDlg, IDD_A04_PLUNO, EM_LIMITTEXT, (WPARAM)A04_NO_LEN, 0L);
        ...
        if (! hMemPluMenu) {    /* Failed Allocate Memory */
            ...
            ParaAllRead(CLASS_PARAPLUNOMENU, (UCHAR *)lpPluData,
                        (MAX_PAGE_NO * FSC_PLU_MICROMAX * sizeof(A04PLU)), 0, &usRetLen);

            /* ----- Insert Page No. to ComboBox ----- */
            for (wI = 0; wI < A04_PAGE_MAX; wI++) {
                wsPepf(szWork, WIDE("%u"), wI + A04_DEF_PAGENO);
                DlgItemSendTextMessage(hDlg, IDD_A04_PAGE, CB_INSERTSTRING, (WPARAM)-1, (LPARAM)(szWork));
            }
...
/* Saratoga End */
	...
	case WM_SETFONT:
		...
		if (hResourceFont) {
			for(int j = IDD_A04_PAGE; j <= IDD_A04_GROUPBOX1; j++)
			{
...
/*** Saratoga ***/
        ...
        case    IDD_A04_PAGE:
            ...
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                wPageNo = (WORD)(A04_DEF_PAGENO + SendDlgItemMessage(hDlg, IDD_A04_PAGE, CB_GETCURSEL, 0, 0L));
...
/* Saratoga */
        ...
        case    IDD_A04_DEL:
            ...
            if (fRet == FALSE) {
                WCHAR    szErr[A04_ERR_LEN + 1] = { 0 };         /* Saratoga */
                WCHAR    szMsg[A04_ERR_LEN + 1] = { 0 };         /* Saratoga */
                WCHAR    szCap[A04_CAP_LEN + 1] = { 0 };         /* Saratoga */
        ...
        case    IDCANCEL:
            ...
            if (HIWORD(wParam) == BN_CLICKED) {
                ...
                if (LOWORD(wParam) == IDOK) {
                    ...
                    ParaAllWrite(CLASS_PARAPLUNOMENU, (UCHAR *)lpPluData,
                                 (USHORT)(MAX_PAGE_NO * FSC_PLU_MICROMAX * sizeof(A04PLU)), 0, &usRetLen);

                    /* ----- Write FSC Data (Prog.#3) for Conversion, 1/23/98 ----- */
                    if (fConvPluData) { /* if conversion is done */

                        ParaAllWrite(CLASS_PARAFSC, (UCHAR *)lpFscData, sizeof(A04FSC), 0, &usRetLen);
...
BOOL A004ZeroChk(HWND hDlg)
{
    ...
    for (USHORT nI = 0; nI < PLU_MAX_DIGIT; nI++) {
        if (szPluNo[nI] == 0x00)
            continue;
...
VOID    A004InitDlg(HWND hDlg)
{
    ...
    DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_PAGE, IDD_A04_STRPAGE);
    DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_PAGELEN, IDD_A04_STRPAGELEN);
    DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_PLUKEY, IDD_A04_STRPLUKEY);
    DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_PLUNO, IDD_A04_STRPLUNO);
    DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_PLUNOLEN, IDD_A04_STRPLUNOLEN);
    DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_DESC, IDD_A04_STRDESC);
    DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_LABEL, IDD_A04_STRDESC2);
}

/*
* ===========================================================================
...
BOOL    ChkPluNo(HWND hDlg)
{
    ...
    DlgItemGetText(hDlg, IDD_A04_PLUNO, szWork, PLU_MAX_DIGIT + 1);
    for (wI = 0; wI < PLU_MAX_DIGIT + 1; wI++) {
        if (szWork[wI] == 0) {
            break;
        } else if ((szWork[wI] < '0') || (szWork[wI] > '9')) {
            fChk = FALSE;
            break;
        }
    }
...
BOOL    WINAPI  A004KeyDlgProc(HWND hDlg, UINT wMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case    WM_INITDIALOG:
        ...
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIXCLR, IDD_A04_FIXCLR);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX00, IDD_A04_FIX00);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX01, IDD_A04_FIX01);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX02, IDD_A04_FIX02);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX03, IDD_A04_FIX03);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX04, IDD_A04_FIX04);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX05, IDD_A04_FIX05);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX06, IDD_A04_FIX06);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX07, IDD_A04_FIX07);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX08, IDD_A04_FIX08);
        DlgItemLoadStringRedrawText(hDlg, hResourceDll, IDS_A04_FIX09, IDD_A04_FIX09);
        ...
        if (! hMemFsc) {    /* Failed to Allocate Memory */
            ...
            ParaAllRead(CLASS_PARAFSC, (UCHAR *)lpFscData, sizeof(A04FSC), 0, &usRetLen);
	...
	case WM_SETFONT:
		...
		if (hResourceFont) {
			for (int j = IDD_A04_KEY01; j <= IDD_A04_KEY165; j++)
			{
				SendDlgItemMessage(hDlg, j, WM_SETFONT, (WPARAM)hResourceFont, 0);
			}
			for (int j = IDD_A04_FIXCLR; j <= IDD_A04_FIX09; j++)
			{
				SendDlgItemMessage(hDlg, j, WM_SETFONT, (WPARAM)hResourceFont, 0);
			}
...
VOID    A004SetPluNo(HWND hDlg, LPA04DATA lpData)
{
    ...
    if (wPluKeyNo) {
        UINT    fMod;                        /* Saratoga */
        WCHAR    szPluNo[PLU_MAX_DIGIT + 1] = { 0 };  /* Saratoga */

        /* ----- Get PLU No. of Specified Keyed PLU Key ----- */
        memcpy(szPluNo, lpData->PluKey[wPageNo - 1][wPluKeyNo - 1].szPluNo, sizeof(szPluNo));
                       
        /* ----- Set PLU No. to EditText ----- */
        szPluNo[PLU_MAX_DIGIT] = 0x00;
        DlgItemRedrawText(hDlg, IDD_A04_PLUNO, szPluNo);
...
BOOL    A004ChkRng(HWND hDlg, WORD wEditID, WORD wMin, WORD wMax)
{
    ...
    if (wData > wMax) {         /* Inputed Data is Out of Range */
        ...
        /* ----- Display Message ----- */
        MessageBeep(MB_ICONEXCLAMATION);
        MessageBoxPopUp(hDlg, szBuff, szCap, MB_OK | MB_ICONEXCLAMATION);
...
VOID    A004ExecAbort(HWND hDlg, short nType)
{
    ...
    if (nType == A04_MEM_ERROR) {
        WCHAR   szCap[PEP_CAPTION_LEN + 1] = { 0 },
            szErr[PEP_LOADSTRING_LEN + 1] = { 0 };
...
VOID    A004SavePluData(HWND hDlg, WPARAM wParam)
{
    ...
    if (wPageNo && wPluKeyNo) {
        ...
        if (wParam == IDD_A04_ADDCHG) {
           ...
           DlgItemGetText(hDlg, IDD_A04_PLUNO, Label.aszScanPlu, PLU_MAX_DIGIT + 1);                    

           /* ----- Get Label Type ----- */
           unPluLab = (UINT)SendDlgItemMessage(hDlg, IDD_A04_LABEL, CB_GETCURSEL, 0, 0L);
           ...
           if ((Label.uchType == LABEL_RANDOM) && (Label.uchLookup != LA_EXE_LOOKUP)) {
              ...
              MessageBoxPopUp(hDlg, szMsg, szCap, MB_OK | MB_ICONEXCLAMATION);
                ...
                memcpy(lpPluData->PluKey[wPageNo - 1][wPluKeyNo - 1].szPluNo, szPluNo, sizeof(szPluNo));
          ...
          /* ----- reset buffer ----- */
          memset(lpPluData->PluKey[wPageNo - 1][wPluKeyNo - 1].szPluNo, 0, (PLU_MAX_DIGIT + 1));
...
VOID    A004ConvPluKey(HWND hDlg, LPA04FSC lpFsc)
{
    ...
    if (! hMemConvPluMenu) {    /* Failed Allocate Memory */
        ...
        /* ----- Display Caution Message ----- */
        MessageBoxPopUp(hDlg, szErr, szCap, MB_OK | MB_ICONQUESTION);
    ...
    /* ----- Read Initial AC 4 Data ----- */
    ParaAllRead(CLASS_PARAPLUNOMENU, (UCHAR *)lpConvPluData,
    ...
    for (wPage = 0; wPage < MAX_PAGE_NO; wPage++) {
        for (wCode = 0; wCode < MAX_FSC_NO; wCode++) {
            ...
            if (!fConv) {
                ...
                memcpy(szPluNo, (lpPluData->PluKey[wPage][wCode].szPluNo), sizeof(szPluNo));
                ...
                /* plu no is not assigned on plu key */
                 || ((bWork == FALSE) && ((UINT)lpFsc->Fsc[wPage].TblFsc[wCode].uchFsc != A04_PLUKEY))) {
                    ...
                    /* ----- Display Caution Message ----- */
                    fRet = MessageBoxPopUp(hDlg, szErr, szCap, MB_YESNO | MB_ICONEXCLAMATION);
            ...
            if ((UINT)lpFsc->Fsc[wPage].TblFsc[wCode].uchFsc == A04_PLUKEY) {
                ...
                memcpy(szPluNo, lpConvPluData->PluKey[wPage][unPluKey].szPluNo, sizeof(szPluNo));
                ...
                if (bNumFlg != FALSE) {   /* ----- PLU number is numeric ----- */
                  ...
                  if (sStat != LABEL_ERR) {
                    ...
                    memcpy(lpPluData->PluKey[wPage][unPluKey].szPluNo, szPluNo, sizeof(szPluNo));
