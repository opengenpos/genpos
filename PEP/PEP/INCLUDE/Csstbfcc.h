...
*   Feb-01-96:T.Nakahata  : R3.1 Initial
...
#define     CLI_MAX_RESDATA     /*477*/ 1400    /* Maximumm request data - These defines should match PIF_LEN_UDATA in R20_pif.h and pifnet.h*/
#define     CLI_MAX_IPDATA      8       /* Maximumm IP address data */
#define     CLI_PASSWORD        10      /* Maximum PC passward */
/*#define     CLI_PLU_MAX_NO      10    */  /* Maximum No of PLU,  R3.0 */
...
#define     CLI_MAX_ETKDATA    (100)    /* Maximumm ETK data  (old 4*250) */
...
#define     CLI_FCCASOPENPICLOA  0x0114  /* Cashier Open  pickup/loan,  Saratoga */
...
#define     CLI_FCTTLINDFINCHK   0x0445  /* TTL individual fin. check, R3.1 */
...
typedef struct {
    ...
    UCHAR   auchCasName[CLI_CASHIERNAME];   /* cashier name */
...
typedef struct {
    ...
    UCHAR   auchWaiName[CLI_WAITERNAME];    /* waiter name */
...
typedef struct {
    ...
    SHORT   sReturn;
    USHORT  usWaiterNo;                     /* waiter number */
    UCHAR   auchWaiName[CLI_CASHIERNAME];   /* waiter name */
    UCHAR   fbStatus;                       /* waiter status */
    UCHAR   uchSecretNo;                    /* secet code */
    USHORT  usStartGCN;                     /* start Guest Check Number */
    USHORT  usEndGCN;                       /* end Guest Check Number */
...
typedef struct {
    ...
    USHORT      usNumber;           /* Cashier/ Waiter/ PLU Number */
...
typedef struct {
    USHORT  usFunCode;
    USHORT  usSeqNo;
    UCHAR   aszMnemonic[CLI_ISPLOG_LEN];
} CLIREQISPLOG2;
...
*------------------------------------------*/
#define CLI_LEN_PORT            5
#define CLI_LEN_OPTION          8
#define CLI_LEN_RMT_PORT        4
#define CLI_LEN_SOFTWARE_ID     17

typedef struct {
    UCHAR       uchTerminal;                        /* Terminal No. */
    UCHAR       auchComPort[CLI_LEN_PORT];          /* Device Type to connect the port */
    ...
    UCHAR       SystemId[18];                       /* System ID */
    UCHAR       ApplicationId[CLI_LEN_SOFTWARE_ID]; /* Application ID */
...
typedef struct {
    ...
    ULONG	usDataLen;
	//USHORT  usDataLen;
    UCHAR   auchEJData[CLI_MAX_EJDATA];
    USHORT  usNumTrans;                    /* V3.3 */
} CLIRESEJ;
...
typedef struct {    /* Add at R3.1 */
    ...
    ULONG   ulOffset;
    USHORT  usDataLen;
    UCHAR   auchEJData[CLI_MAX_EJREPTDATA];
...
typedef struct {
    ...
    UCHAR   uchJobCode;
    USHORT  usFieldNo;
    UCHAR   auchNewJob[CLI_NEW_ETK_JOB];
    UCHAR   auchEmployeeName[CLI_EMPLOYEE_NAME];    /* Add R3.1 */
} CLIREQETK;
...
typedef struct {
    ...
    SHORT   sTMStatus;
    USHORT  usDataLen;
    UCHAR   auchETKData[CLI_MAX_ETKDATA];
    UCHAR   auchEmployeeName[CLI_EMPLOYEE_NAME];    /* Add R3.1 */
} CLIRESETK;
...
typedef struct {
    ...
    UCHAR   auchPara[sizeof(ECMCOND)];  /* Parameter */
} CLIREQPLUECM;
...
typedef struct {
    ...
    UCHAR   aucMnemonic[20];            /* mnemonics                */
    UCHAR   aucStatus[12];              /* status                   */
